...
// ver0.4.7: ボス表示整理・火炎ブレスタグカウント・ボスランダム出現。
...
const GAME_VERSION = (window.APP_VERSION || '0.4.7');
...
function handleHeroDeath(){
  ...
  if(darkSaintDefeat){
    // ver.0.4.7: 100レベルごとの暗黒剣聖は、勝敗に関係なく通常敵レベル+1。
    if(state.darkSwordSaintReturn && state.darkSwordSaintReturn.milestoneDarkSaint){
...
function pickEnemy(){
  // ver.0.4.7: 通常敵レベルは撃破時抽選で上昇。ボスはLv10刻み予約からランダム出現。
...
function scheduleBossAfterNormalDefeat(e){
  // ver.0.4.7: 雑魚Lv10刻みで、種族ボスからランダム出現。レベルは撃破した雑魚と共通。
  try{
    ...
    console.error('[MBH0.4.7 scheduleBossAfterNormalDefeat]', err);
...
function makeScaledEnemy(base, forceLevel=null){
  ...
  if(forceLevel){
    ...
    // ver.0.4.7: 敵レベルは撃破数で自動上昇させず、撃破時の抽選/ボス撃破で更新する。
  ...
  // ver.0.4.7: 敵ATK/DEFのレベル成長が弱すぎたため、HPとは別の成長係数へ分離。
...
function forceSpawnDarkSwordSaint(){
  // ver0.4.7: 即時召喚ではなく、次の敵として暗黒剣聖を予約する
...
function enemyAttack(now){
  ...
  // ver0.4.7: 火の精霊・火の精霊王の通常攻撃は必ず火属性。
...
function tryDarkSwordDanceRevive(){
  ...
  // ver.0.4.7: 暗黒剣舞でリセットするのは死線の剣舞の発動回数だけ。
  // 発動中の死線の剣舞そのものは解除しない。
  state.deathDanceBattleCount = 0;
...
function updateEnemyLevelProgressionOnDefeat(e){
  // ver.0.4.7: 雑魚撃破は10%で敵レベル+1。ボス撃破は確定+1。
  // 暗黒剣聖は通常敵進行に影響させない。
  try{
    if(!e) return;
    ...
    console.error('[MBH0.4.7 updateEnemyLevelProgressionOnDefeat]', err);
  }
}
...
function enemyDefeated(){
  ...
  // ver.0.4.7: 強化石ドロップは廃止。装備はドロップ時に敵Lv依存の+値が付く。
...
function checkLevelUp(){
  // ver0.4.7: 主人公の次Lv必要経験値を現行値の50%として扱う。
...
function darkEquipLevelFromSource(levelOverride){
  ...
  // ver.0.4.7: 闇装備は暗黒剣聖の独立Lv1を通常敵Lv100相当として扱う。
...
init();
// ver0.4.7: disable old stacked v94 menu/debug tap binding; clean patch below owns UI events.
...
/* ver0.4.7 clean stabilization patch: single source, no stacked UI injection */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const byId = (id)=>document.getElementById(id);
  function safe(fn){ try{ return fn && fn(); }catch(e){ console.warn('[ver0.4.7]', e); return null; } }
  ...
  function bindCleanDebug(){
    // ver0.4.7: debug panel must be idempotent. Clone once to remove stacked click/pointer handlers.
...
/* ver.0.4.7: audio restore - use lexical state/audio functions, not window fallback */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH0.4.7]', e); } }
  function setVersion(){
    window.GAME_VERSION=BUILD;
...
/* ver.0.4.7: inventory filter and log viewport final fix */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
...
/* ver.0.4.7: mobile tap recovery only. PC mouse behavior is untouched. */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  ...
  function runMobileTap(e){
    ...
    setTimeout(() => {
      try {
        ...
        console.warn('[MBH0.4.7 mobile tap]', err);
      }
    }, 0);
  }
  function boot(){
...
/* ver.0.4.7: mobile equipment tap + mobile background audio pause only */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
...
/* ver.0.4.7: enhancement removal + enemy-level drop plus finalizer */
(function(){
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH0.4.7]', e); } }
  safe(()=>{
...
/* ver.0.4.7: do not stop BGM on ordinary outside click/blur */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH0.4.7]', e); } }
...
/* ver.0.4.7: battle log scroll-position preserve + lower padding fix */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
...
/* ver.0.4.7: strong/named variants, drop source, quality/luck bonuses, deathdance count-only reset補強 */
(function(){
  'use strict';
  const APP_VERSION = '0.4.7';
  const PREFIXES = ['狡猾な','獰猛な','蛮勇な','エリート','頂点の','原点の','キラー'];
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH 0.4.7]', e); } }
...
/* ver.0.4.7: 敵防御を軽減式へ変更、敵レベル経験値制御、Lv100までの戦闘テンポ調整 */
(function(){
  'use strict';
  const APP_VERSION = '0.4.7';
...
/* ver.0.4.7: 敵Lv経験値完全固定 + 敵攻撃を防御軽減式へ変更 */
(function(){
  'use strict';
  const APP_VERSION = '0.4.7';
  ...
  // 敵ステータス再調整：HPは0.4.7方針維持、攻撃力は防御装備が意味を持つ範囲まで上昇
...
/* ver.0.4.7: 暗黒剣聖の独立Lv表示 + 主人公必要経験値固定 */
(function(){
  'use strict';
  const APP_VERSION = '0.4.7';
...
/* ver.0.4.7 hotfix: hero debug stats, dark equipment list visibility, low-level enemy HP trim */
(function(){
  'use strict';
  const APP_VERSION = '0.4.7';
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH 0.4.7 hotfix]', e); } }
  function esc(s){ return (typeof escapeHtml === 'function') ? escapeHtml(s) : String(s).replace(/[&<>"']/g, ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])); }
  function pct(v){ return Math.round((Number(v)||0)*100)+'%'; }
...
(function(){
  ...
  function safe(fn){ try{ return fn(); }catch(e){ console.warn('[MBH 0.4.7]', e); return null; } }
  function pct(v){ return Math.round((Number(v)||0)*100) + '%'; }
  function num(v){ return Math.floor(Number(v)||0).toLocaleString(); }
...
/* ver.0.4.7: final XP single-source correction */
(function(){
  'use strict';
  const APP_VERSION = '0.4.7';
  const HERO_XP_NEXT = 1000;
  function safe(fn){ try{ return fn && fn(); }catch(e){ console.warn('[MBH 0.4.7 final]', e); } }
...
/* ver.0.4.7: variant buff list, named regen position, legal links only inside menu, boss low-level tuning, BGM accidental-stop guard */
(function(){
  'use strict';
  const BUILD = (window.APP_VERSION || window.GAME_VERSION || '0.4.7');
  function safe(fn){ try{return fn && fn();}catch(e){ console.error('[MBH0.4.7]', e); return null; } }
  ...
  // 0.4.7の中央上表示をやめ、敵カードの左外へ出す専用クラスにする。
...
/* ver.0.4.7: enemy missing watchdog + debug enemy level/variant restore */
(function(){
  'use strict';
  const BUILD = '0.4.7';
...
/* ver.0.4.7: startup-safe fixed menu layout, footer, log type, debug enemy controls */
(function(){
  'use strict';
  const BUILD='0.4.7';
...
/* ver.0.4.7: root cache-bust, fixed split layouts, fixed footer, fixed log categories */
(function(){
  'use strict';
  const BUILD = '0.4.7';
  ...
  const byId = (id)=>document.getElementById(id);
  const safe = (fn)=>{ try{return fn();}catch(e){ console.error('[MBH 0.4.7]', e); } };
...
/* ver.0.4.7: footer isolated from log redraw + final log type lock */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const qs=(s,r=document)=>r.querySelector(s);
...
/* ver.0.4.7: boss BGM + startup menu closed final lock */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const qs=(s,r=document)=>r.querySelector(s);
...
/* ver.0.4.7: final stable menu/footer/effects/log/sell-exp lock */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const $=(s,r=document)=>r.querySelector(s);
...
/* ver.0.4.7: menu initial-close hard lock.
...
/* ver.0.4.7: final layout/log/effect/menu stability lock */
(function(){
  'use strict';
  const BUILD='0.4.7';
...
/* ver.0.4.7: final stable layout, no insertBefore loop, null enemy guard, log category hard fix */
(function(){
  'use strict';
  const APP_VERSION = '0.4.7';
  const $ = (id)=>document.getElementById(id);
  ...
  function safe(fn,label){ try{return fn();}catch(e){ console.error('[MBH 0.4.7 '+(label||'')+']', e); } }
...
/* MBH ver.0.4.7: 軽量化・ログ撤去・メニューDOM固定 */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const $=(id)=>document.getElementById(id);
  const $$=(sel,root=document)=>Array.from(root.querySelectorAll(sel));
  const safe=(fn)=>{ try{return fn&&fn();}catch(e){ console.warn('[MBH 0.4.7]', e); } };
...
/* MBH ver.0.4.7: menu layout rule finalizer */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const $=(id)=>document.getElementById(id);
  const $$=(sel,root=document)=>Array.from(root.querySelectorAll(sel));
  const safe=(fn)=>{ try{return fn&&fn();}catch(e){ console.warn('[MBH 0.4.7 layout]', e); } };
  ...
  function installCss041Final(){
    ...
    st.textContent=`
      html[data-build-version="0.4.7"] .log-panel,
      html[data-build-version="0.4.7"] #log,
      html[data-build-version="0.4.7"] #logFilterBar,
      html[data-build-version="0.4.7"] [data-menu-page="log"]{display:none!important;}

      html[data-build-version="0.4.7"] .side-panel{
        box-sizing:border-box!important;
      ...
      html[data-build-version="0.4.7"] .side-panel *{box-sizing:border-box!important;}
      html[data-build-version="0.4.7"] .side-panel>.mobile-menu-tabs{
        display:grid!important;
      ...
      html[data-build-version="0.4.7"] .side-panel>.mobile-menu-tabs button{min-width:0!important;width:100%!important;}

      html[data-build-version="0.4.7"] .menu-col,
      html[data-build-version="0.4.7"] .menu-left,
      html[data-build-version="0.4.7"] .menu-right,
      html[data-build-version="0.4.7"] .menu-main{display:contents!important;width:auto!important;height:auto!important;max-width:none!important;max-height:none!important;}

      html[data-build-version="0.4.7"] #mbhMenuContent041,
      html[data-build-version="0.4.7"] #mbhMenuContent040,
      html[data-build-version="0.4.7"] #mbhMenuContent039,
      html[data-build-version="0.4.7"] .menu-content-fixed{
        width:100%!important;
      ...
      html[data-build-version="0.4.7"] .side-panel .panel{
        width:100%!important;
        height:100%!important;
        min-width:0!important;
        min-height:0!important;
      ...
      html[data-build-version="0.4.7"] .side-panel .panel:not(.active-page){display:none!important;}
      html[data-build-version="0.4.7"] .side-panel .panel.active-page{display:flex!important;flex-direction:column!important;}

      html[data-build-version="0.4.7"] .status-layout,
      html[data-build-version="0.4.7"] .equip-layout{
      ...
      html[data-build-version="0.4.7"] .status-left,
      html[data-build-version="0.4.7"] .status-right,
      html[data-build-version="0.4.7"] .equip-left,
      html[data-build-version="0.4.7"] .equip-right{
        min-width:0!important;
        min-height:0!important;
        max-width:none!important;
        overflow:hidden!important;
      ...
      html[data-build-version="0.4.7"] .hero-stats>h2,
      html[data-build-version="0.4.7"] .equip-panel>h2{flex:0 0 auto!important;}
      html[data-build-version="0.4.7"] .status-left .stat-grid{flex:0 0 auto!important;}
      html[data-build-version="0.4.7"] .status-special-effects,
      html[data-build-version="0.4.7"] .monster-records,
      html[data-build-version="0.4.7"] .equip-effect-totals{
        flex:1 1 auto!important;
        min-height:0!important;
        max-height:none!important;
        overflow:hidden!important;
      ...
      html[data-build-version="0.4.7"] .status-special-effects{margin-top:10px!important;}
      html[data-build-version="0.4.7"] .monster-records,
      html[data-build-version="0.4.7"] .equip-effect-totals{margin-top:0!important;}
      html[data-build-version="0.4.7"] .effect-scroll,
      html[data-build-version="0.4.7"] .monster-record-list{
        width:100%!important;
        flex:1 1 auto!important;
        min-height:0!important;
        max-height:none!important;
      ...
      html[data-build-version="0.4.7"] .effect-scroll::-webkit-scrollbar,
      html[data-build-version="0.4.7"] .monster-record-list::-webkit-scrollbar,
      html[data-build-version="0.4.7"] #inventory::-webkit-scrollbar,
      html[data-build-version="0.4.7"] .equip-list::-webkit-scrollbar{width:9px!important;height:9px!important;}
      html[data-build-version="0.4.7"] .effect-scroll::-webkit-scrollbar-track,
      html[data-build-version="0.4.7"] .monster-record-list::-webkit-scrollbar-track,
      html[data-build-version="0.4.7"] #inventory::-webkit-scrollbar-track,
      html[data-build-version="0.4.7"] .equip-list::-webkit-scrollbar-track{background:#100905!important;border-radius:9px!important;}
      html[data-build-version="0.4.7"] .effect-scroll::-webkit-scrollbar-thumb,
      html[data-build-version="0.4.7"] .monster-record-list::-webkit-scrollbar-thumb,
      html[data-build-version="0.4.7"] #inventory::-webkit-scrollbar-thumb,
      html[data-build-version="0.4.7"] .equip-list::-webkit-scrollbar-thumb{background:linear-gradient(#d7a340,#7b5519)!important;border-radius:9px!important;border:1px solid #2b1a07!important;}

      html[data-build-version="0.4.7"] .equip-list{
        flex:1 1 auto!important;
        min-height:0!important;
        width:100%!important;
        max-width:none!important;
      ...
      html[data-build-version="0.4.7"] .equip-actions-fixed,
      html[data-build-version="0.4.7"] #bestEquipBtnEquip,
      html[data-build-version="0.4.7"] #upgradeBtn{display:none!important;}
      html[data-build-version="0.4.7"] .equip-left>#bestEquipBtn{flex:0 0 auto!important;display:block!important;width:100%!important;margin:8px 0 0!important;}

      html[data-build-version="0.4.7"] .inventory-panel.active-page{
        display:flex!important;
        flex-direction:column!important;
        width:100%!important;
        height:100%!important;
      ...
      html[data-build-version="0.4.7"] .inventory-panel h2{flex:0 0 auto!important;}
      html[data-build-version="0.4.7"] .inventory-actions,
      html[data-build-version="0.4.7"] .inventory-filter-bar{flex:0 0 auto!important;width:100%!important;max-width:none!important;}
      html[data-build-version="0.4.7"] #inventory{
        flex:1 1 auto!important;
        min-height:0!important;
        width:100%!important;
        max-width:none!important;
      ...
      html[data-build-version="0.4.7"] .menu-footer-fixed,
      html[data-build-version="0.4.7"] .side-panel>.legal-links,
      html[data-build-version="0.4.7"] .side-panel>.menu-footer{
        flex:0 0 auto!important;
        display:grid!important;
        grid-template-columns:repeat(3,minmax(0,1fr))!important;
        gap:8px!important;
      ...
      html[data-build-version="0.4.7"] .panel .legal-links,
      html[data-build-version="0.4.7"] .panel .menu-footer{display:none!important;}

      html[data-build-version="0.4.7"] body.menu-open .flee-btn,
      html[data-build-version="0.4.7"] body.menu-open .expbar{display:none!important;}

      @media(min-width:1280px) and (min-height:701px) and (pointer:fine){
        html[data-build-version="0.4.7"] .layout{grid-template-columns:minmax(780px,1fr) minmax(560px,760px)!important;}
        html[data-build-version="0.4.7"] .side-panel{position:static!important;display:grid!important;width:auto!important;height:auto!important;background:none!important;border:0!important;padding:0!important;box-shadow:none!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;}
        html[data-build-version="0.4.7"] #equipToggleBtn{display:none!important;}
      }
      @media(max-width:1279px),(max-height:700px),(pointer:coarse){
        html[data-build-version="0.4.7"] .layout{grid-template-columns:1fr!important;}
        html[data-build-version="0.4.7"] #equipToggleBtn{display:inline-flex!important;}
        html[data-build-version="0.4.7"] body:not(.mbh-menu-open) .side-panel{display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important;}
        html[data-build-version="0.4.7"] body.mbh-menu-open .side-panel{display:grid!important;position:fixed!important;left:8px!important;right:8px!important;top:calc(var(--mbh-topbar-h,58px) + 8px)!important;bottom:8px!important;width:auto!important;height:auto!important;z-index:70!important;background:rgba(0,0,0,.94)!important;border:2px solid #c99b39!important;padding:8px!important;box-shadow:0 0 35px #000!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;}
      }
      @media(max-width:760px){
        html[data-build-version="0.4.7"] .status-layout,
        html[data-build-version="0.4.7"] .equip-layout{grid-template-columns:1fr!important;overflow-y:auto!important;}
        html[data-build-version="0.4.7"] .side-panel>.mobile-menu-tabs{grid-template-columns:repeat(3,minmax(0,1fr))!important;}
        html[data-build-version="0.4.7"] .side-panel>.legal-links,
        html[data-build-version="0.4.7"] .side-panel>.menu-footer{grid-template-columns:1fr!important;}
        html[data-build-version="0.4.7"] .equip-list{grid-template-columns:1fr!important;}
        html[data-build-version="0.4.7"] #inventory{grid-template-columns:repeat(auto-fill,minmax(120px,1fr))!important;}
      }
    `;
  }
...
/* MBH ver.0.4.7: 固有能力表示の固定化 + レアリティ色付け + 戦闘中の一時効果を表示しない */
(function(){
  'use strict';
  const BUILD='0.4.7';
  ...
  function safe(fn){ try{return fn&&fn();}catch(e){ console.warn('[MBH 0.4.7]', e); return null; } }
  function $(id){return document.getElementById(id);}
  ...
  function installCss(){
    ...
    st.textContent=`
      html[data-build-version="0.4.7"] .effect-row.ability-option span,
      html[data-build-version="0.4.7"] .effect-row.ability-option b{color:var(--effect-color)!important;text-shadow:0 0 8px color-mix(in srgb, var(--effect-color) 55%, transparent)!important;}
      html[data-build-version="0.4.7"] .effect-row.basic-option span{color:#e8d0a2!important;}
      html[data-build-version="0.4.7"] .effect-row.basic-option b{color:#fff7c8!important;}
      html[data-build-version="0.4.7"] .equip small .item-ability-option{color:var(--item-ability-color)!important;text-shadow:0 0 7px color-mix(in srgb, var(--item-ability-color) 50%, transparent)!important;font-weight:800!important;}
      html[data-build-version="0.4.7"] .equip small .item-basic-option{color:#c8b98e!important;}
    `;
    document.head.appendChild(st);
  }
...
/* MBH ver.0.4.7: version single-source, stable effect panels, debug/menu layout lock, named regen display */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const BASIC_COLOR='#f7ead0';
  ...
  function safe(fn){ try{return fn&&fn();}catch(e){ console.warn('[MBH0.4.7]', e); return null; } }
  function $(id){ return document.getElementById(id); }
  function esc(s){ return String(s??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])); }
  function pct(v,signed=false){ const n=Math.round(Number(v||0)*100); return (signed&&n>0?'+':'')+n+'%'; }
  function num(v,signed=true){ const n=Math.floor(Number(v||0)); return (signed&&n>0?'+':'')+n.toLocaleString(); }
  ...
  function installCss(){
    ...
    st.textContent=`
      ...
      html[data-build-version="0.4.7"] .effect-row.basic-option span{color:#e8d0a2!important;}
      html[data-build-version="0.4.7"] .effect-row.basic-option b{color:#fff7c8!important;}
      html[data-build-version="0.4.7"] #statusSpecialEffects .effect-scroll,
      html[data-build-version="0.4.7"] #equipEffectTotals .effect-scroll{scrollbar-color:#9d742a #111!important;scrollbar-width:thin!important;overflow:auto!important;}
      html[data-build-version="0.4.7"] #statusSpecialEffects .effect-scroll::-webkit-scrollbar,
      html[data-build-version="0.4.7"] #equipEffectTotals .effect-scroll::-webkit-scrollbar{width:8px!important;}
      html[data-build-version="0.4.7"] #statusSpecialEffects .effect-scroll::-webkit-scrollbar-thumb,
      html[data-build-version="0.4.7"] #equipEffectTotals .effect-scroll::-webkit-scrollbar-thumb{background:#8b6628!important;border-radius:8px!important;}
      html[data-build-version="0.4.7"] #statusSpecialEffects .effect-scroll::-webkit-scrollbar-track,
      html[data-build-version="0.4.7"] #equipEffectTotals .effect-scroll::-webkit-scrollbar-track{background:#120b06!important;}
      @media (min-width:1280px) and (min-height:701px){
        html[data-build-version="0.4.7"] .side-panel{position:static!important;display:grid!important;grid-template-rows:auto minmax(0,1fr) auto!important;grid-template-columns:1fr!important;overflow:hidden!important;}
        html[data-build-version="0.4.7"] .side-panel .mobile-menu-tabs{grid-row:1!important;position:relative!important;top:auto!important;left:auto!important;right:auto!important;display:flex!important;z-index:2!important;}
        html[data-build-version="0.4.7"] #debugPanel:not(.hidden){position:fixed!important;right:18px!important;top:70px!important;z-index:12000!important;max-height:calc(100vh - 88px)!important;overflow:auto!important;}
        html[data-build-version="0.4.7"] #debugPanel:not(.hidden) ~ *{position:static!important;}
      }
    `;
    document.head.appendChild(st);
  }
...
/* MBH ver.0.4.7: final menu full-page layout + debug next enemy queue + spawn overwrite guard */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const $=(id)=>document.getElementById(id);
  const qs=(s,r=document)=>r.querySelector(s);
  const qsa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const safe=(fn)=>{ try{ return fn&&fn(); }catch(e){ console.warn('[MBH0.4.7]', e); return null; } };
  ...
  function ensureCss044(){
    ...
    st.textContent=`
      ...
      html[data-build-version="0.4.7"] .side-panel>.mobile-menu-tabs button{flex:1 1 0!important;min-width:0!important;white-space:nowrap!important;}
      html[data-build-version="0.4.7"] .side-panel>.menu-col,
      html[data-build-version="0.4.7"] .side-panel>.menu-main,
      html[data-build-version="0.4.7"] .side-panel>.menu-left,
      html[data-build-version="0.4.7"] .side-panel>.menu-right{display:none!important;}
      html[data-build-version="0.4.7"] .side-panel>.menu-content-fixed,
      html[data-build-version="0.4.7"] #mbhMenuContent044{
      ...
      html[data-build-version="0.4.7"] #mbhMenuContent044>.panel{
      ...
      html[data-build-version="0.4.7"] #mbhMenuContent044>.panel:not(.active-page){display:none!important;}
      html[data-build-version="0.4.7"] #mbhMenuContent044>.panel.active-page{display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.4.7"] .hero-stats>.status-layout,
      html[data-build-version="0.4.7"] .equip-panel>.equip-layout{
      ...
      html[data-build-version="0.4.7"] .status-right,
      html[data-build-version="0.4.7"] .equip-left,
      html[data-build-version="0.4.7"] .equip-right{min-width:0!important;min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.4.7"] .status-special-effects,
      html[data-build-version="0.4.7"] .monster-records,
      html[data-build-version="0.4.7"] .equip-effect-totals{flex:1 1 auto!important;min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.4.7"] .effect-scroll,
      html[data-build-version="0.4.7"] .monster-record-list,
      html[data-build-version="0.4.7"] .equip-list,
      html[data-build-version="0.4.7"] .inventory{min-height:0!important;overflow-y:auto!important;overflow-x:hidden!important;scrollbar-color:#9d742a #111!important;scrollbar-width:thin!important;}
      html[data-build-version="0.4.7"] .effect-scroll::-webkit-scrollbar,
      html[data-build-version="0.4.7"] .monster-record-list::-webkit-scrollbar,
      html[data-build-version="0.4.7"] .equip-list::-webkit-scrollbar,
      html[data-build-version="0.4.7"] .inventory::-webkit-scrollbar{width:8px!important;}
      html[data-build-version="0.4.7"] .effect-scroll::-webkit-scrollbar-thumb,
      html[data-build-version="0.4.7"] .monster-record-list::-webkit-scrollbar-thumb,
      html[data-build-version="0.4.7"] .equip-list::-webkit-scrollbar-thumb,
      html[data-build-version="0.4.7"] .inventory::-webkit-scrollbar-thumb{background:#8b6628!important;border-radius:8px!important;}
      html[data-build-version="0.4.7"] .effect-scroll::-webkit-scrollbar-track,
      html[data-build-version="0.4.7"] .monster-record-list::-webkit-scrollbar-track,
      html[data-build-version="0.4.7"] .equip-list::-webkit-scrollbar-track,
      html[data-build-version="0.4.7"] .inventory::-webkit-scrollbar-track{background:#120b06!important;}
      html[data-build-version="0.4.7"] .inventory-panel.active-page{display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.4.7"] .inventory-panel .inventory{flex:1 1 auto!important;width:100%!important;max-height:none!important;display:grid!important;grid-template-columns:repeat(auto-fill,minmax(96px,1fr))!important;align-content:start!important;}
      html[data-build-version="0.4.7"] .menu-footer{grid-row:3!important;position:relative!important;display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:8px!important;width:100%!important;margin:0!important;z-index:4!important;}
      html[data-build-version="0.4.7"] #bestEquipBtnEquip{display:none!important;}
      @media (min-width:1280px) and (min-height:701px){
        html[data-build-version="0.4.7"] #equipToggleBtn{display:none!important;}
        html[data-build-version="0.4.7"] .side-panel{position:static!important;width:auto!important;background:none!important;border:0!important;box-shadow:none!important;padding:0!important;}
        html[data-build-version="0.4.7"] #debugPanel:not(.hidden){position:fixed!important;right:18px!important;top:70px!important;z-index:12000!important;max-height:calc(100vh - 88px)!important;overflow:auto!important;}
      }
      @media (max-width:1279px),(max-height:700px){
        html[data-build-version="0.4.7"] #equipToggleBtn{display:inline-block!important;}
        html[data-build-version="0.4.7"] .side-panel{display:none!important;position:fixed!important;left:8px!important;right:8px!important;top:calc(var(--mbh-topbar-h,58px) + 8px)!important;bottom:8px!important;width:auto!important;max-width:calc(100vw - 16px)!important;z-index:70!important;background:rgba(0,0,0,.94)!important;border:2px solid #c99b39!important;padding:8px!important;box-shadow:0 0 35px #000!important;}
        html[data-build-version="0.4.7"] .side-panel.open{display:grid!important;}
      }
      @media (max-width:760px){
        html[data-build-version="0.4.7"] .hero-stats>.status-layout,
        html[data-build-version="0.4.7"] .equip-panel>.equip-layout{grid-template-columns:1fr!important;overflow-y:auto!important;}
        html[data-build-version="0.4.7"] .menu-footer{grid-template-columns:1fr!important;}
      }
    `;
    document.head.appendChild(st);
  }
...
/* MBH ver.0.4.7: final build lock + visible menu tabs repair */
(function(){
  'use strict';
  const BUILD='0.4.7';
  const $=(id)=>document.getElementById(id);
...
/* MBH ver.0.4.7: safe menu layer fix - no MutationObserver loop */
(function(){
  'use strict';
  const BUILD='0.4.7';
  ...
  function syncVersion047(){
    ...
    try{
      ...
      $$('.debug-version').forEach(el=>{ el.textContent='Build: ver.'+BUILD; });
      $$('.debug-trace-title').forEach(el=>{ el.textContent='進行デバッグログ ver.'+BUILD; });
    }catch(e){ console.warn('[MBH0.4.7 syncVersion]', e); }
  ...
  function installCss047(){
    ...
    st.textContent=`
      html[data-build-version="0.4.7"] .layout{position:relative!important;z-index:1!important;}
      html[data-build-version="0.4.7"] .battle-panel{position:relative!important;z-index:1!important;}
      html[data-build-version="0.4.7"] .side-panel{position:relative!important;z-index:9000!important;pointer-events:auto!important;}
      html[data-build-version="0.4.7"] .side-panel.open{position:fixed!important;z-index:20000!important;pointer-events:auto!important;}
      html[data-build-version="0.4.7"] #debugPanel:not(.hidden){z-index:21000!important;pointer-events:auto!important;}
      html[data-build-version="0.4.7"] .mobile-menu-tabs{display:flex!important;width:100%!important;min-height:40px!important;gap:8px!important;flex:0 0 auto!important;}
      html[data-build-version="0.4.7"] .mobile-menu-tabs button,
      html[data-build-version="0.4.7"] [data-menu-page]{display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:1 1 0!important;min-width:88px!important;min-height:40px!important;height:40px!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;white-space:nowrap!important;}
      html[data-build-version="0.4.7"] .menu-footer{position:relative!important;z-index:9001!important;pointer-events:auto!important;}
      html[data-build-version="0.4.7"] .menu-footer button{min-width:0!important;pointer-events:auto!important;}
      @media (min-width:1280px) and (min-height:701px){
        html[data-build-version="0.4.7"] .side-panel{display:grid!important;position:relative!important;grid-column:2!important;width:100%!important;height:100%!important;background:none!important;border:0!important;box-shadow:none!important;padding:0!important;}
      }
      @media (max-width:1279px),(max-height:700px){
        html[data-build-version="0.4.7"] .side-panel:not(.open){display:none!important;pointer-events:none!important;}
        html[data-build-version="0.4.7"] .side-panel.open{display:grid!important;left:8px!important;right:8px!important;top:calc(var(--mbh-topbar-h,58px) + 8px)!important;bottom:8px!important;width:auto!important;}
      }
    `;
    document.head.appendChild(st);
  }
