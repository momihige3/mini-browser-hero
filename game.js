...
// ver0.5.3: ボス表示整理・火炎ブレスタグカウント・ボスランダム出現。
...
const GAME_VERSION = (window.APP_VERSION || '0.5.3');
...
function spawnWeakEnemyAfterEscape(){
  ...
  spawnEnemy(false);
  setBgmMode(isDarkSwordSaint() ? 'dark_sword_saint' : (state.enemy && state.enemy.type === 'ボス' ? 'boss' : 'normal'));
  renderAll();
  scheduleSave();
}
...
function handleHeroDeath(){
  ...
  if(darkSaintDefeat){
    // ver.0.5.3: 100レベルごとの暗黒剣聖は、勝敗に関係なく通常敵レベル+1。
    if(state.darkSwordSaintReturn && state.darkSwordSaintReturn.milestoneDarkSaint){
...
function pickEnemy(){
  // ver.0.5.3: 通常敵レベルは撃破時抽選で上昇。ボスはLv10刻み予約からランダム出現。
...
function scheduleBossAfterNormalDefeat(e){
  // ver.0.5.3: 雑魚Lv10刻みで、種族ボスからランダム出現。レベルは撃破した雑魚と共通。
  try{
    ...
    console.error('[MBH0.5.3 scheduleBossAfterNormalDefeat]', err);
...
function makeScaledEnemy(base, forceLevel=null){
  ...
  if(forceLevel){
    ...
    // ver.0.5.3: 敵レベルは撃破数で自動上昇させず、撃破時の抽選/ボス撃破で更新する。
  ...
  // ver.0.5.3: 敵ATK/DEFのレベル成長が弱すぎたため、HPとは別の成長係数へ分離。
...
function forceSpawnDarkSwordSaint(){
  // ver0.5.3: 即時召喚ではなく、次の敵として暗黒剣聖を予約する
...
function enemyAttack(now){
  ...
  // ver0.5.3: 火の精霊・火の精霊王の通常攻撃は必ず火属性。
...
function tryDarkSwordDanceRevive(){
  ...
  // ver.0.5.3: 暗黒剣舞でリセットするのは死線の剣舞の発動回数だけ。
  // 発動中の死線の剣舞そのものは解除しない。
  state.deathDanceBattleCount = 0;
...
function updateEnemyLevelProgressionOnDefeat(e){
  // ver.0.5.3: 雑魚撃破は10%で敵レベル+1。ボス撃破は確定+1。
  // 暗黒剣聖は通常敵進行に影響させない。
  try{
    if(!e) return;
    ...
    console.error('[MBH0.5.3 updateEnemyLevelProgressionOnDefeat]', err);
  }
}
...
function enemyDefeated(){
  ...
  // ver.0.5.3: 強化石ドロップは廃止。装備はドロップ時に敵Lv依存の+値が付く。
...
function checkLevelUp(){
  // ver0.5.3: 主人公の次Lv必要経験値を現行値の50%として扱う。
...
function ensureBgmAudio(){
  ...
  if(!state.bossBgm){
    state.bossBgm = new Audio('Before_the_Gate_Falls.mp3');
    state.bossBgm.loop = true;
    state.bossBgm.preload = 'auto';
  }
...
function updateBgmVolume(){
  ...
  if(state.bossBgm) state.bossBgm.volume = v;
...
function stopAllBgm(){
  ...
  if(state.bossBgm){ state.bossBgm.pause(); state.bossBgm.currentTime = 0; }
...
function stopAllAudioForMute(){
  ...
  stopHtmlAudio(state.bossBgm, false);
...
function pauseBgmForPageHidden(){
  ...
  if(state.bossBgm) state.bossBgm.pause();
...
function playBgm(){
  ...
  }else if(state.bgmMode === 'boss'){
    if(state.normalBgm) state.normalBgm.pause();
    if(state.swordDanceBgm){ state.swordDanceBgm.pause(); try{ state.swordDanceBgm.currentTime = 0; }catch(e){} }
    if(state.darkSwordSaintBgm){ state.darkSwordSaintBgm.pause(); try{ state.darkSwordSaintBgm.currentTime = 0; }catch(e){} }
    if(state.bossBgm){
      if(state.bossBgm.ended || (state.bossBgm.paused && (!state.bossBgm.currentTime || state.bossBgm.currentTime <= 0.05))){
        try{ state.bossBgm.currentTime = 0; }catch(e){}
      }
      safePlayAudio(state.bossBgm);
    }
  }else{
    if(state.swordDanceBgm){ state.swordDanceBgm.pause(); try{ state.swordDanceBgm.currentTime = 0; }catch(e){} }
    if(state.darkSwordSaintBgm){ state.darkSwordSaintBgm.pause(); try{ state.darkSwordSaintBgm.currentTime = 0; }catch(e){} }
    if(state.bossBgm){ state.bossBgm.pause(); try{ state.bossBgm.currentTime = 0; }catch(e){} }
...
function darkEquipLevelFromSource(levelOverride){
  ...
  // ver.0.5.3: 闇装備は暗黒剣聖の独立Lv1を通常敵Lv100相当として扱う。
...
init();
// ver0.5.3: disable old stacked v94 menu/debug tap binding; clean patch below owns UI events.
...
/* ver0.5.3 clean stabilization patch: single source, no stacked UI injection */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const byId = (id)=>document.getElementById(id);
  function safe(fn){ try{ return fn && fn(); }catch(e){ console.warn('[ver0.5.3]', e); return null; } }
  ...
  function bindCleanDebug(){
    // ver0.5.3: debug panel must be idempotent. Clone once to remove stacked click/pointer handlers.
...
/* ver.0.5.3: audio restore - use lexical state/audio functions, not window fallback */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH0.5.3]', e); } }
  function setVersion(){
    window.GAME_VERSION=BUILD;
...
/* ver.0.5.3: inventory filter and log viewport final fix */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
...
/* ver.0.5.3: mobile tap recovery only. PC mouse behavior is untouched. */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  ...
  function runMobileTap(e){
    ...
    setTimeout(() => {
      try {
        ...
        console.warn('[MBH0.5.3 mobile tap]', err);
      }
    }, 0);
  }
  function boot(){
...
/* ver.0.5.3: mobile equipment tap + mobile background audio pause only */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
  ...
  function installMobileAudioPause(){
    ...
    function pauseMobileAudio(){
      ...
      safe(()=>{
        ...
        state.bgmPausedByVisibility=true;
        [state.normalBgm,state.swordDanceBgm,state.darkSwordSaintBgm,state.bossBgm,state.darkSwordSaintVoice].forEach(a=>{ try{ if(a) a.pause(); }catch(e){} });
      });
    }
...
/* ver.0.5.3: enhancement removal + enemy-level drop plus finalizer */
(function(){
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH0.5.3]', e); } }
  safe(()=>{
...
/* ver.0.5.3: do not stop BGM on ordinary outside click/blur */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH0.5.3]', e); } }
  ...
  function directPauseAudio(){
    safe(()=>{
      if(typeof state === 'undefined') return;
      state.bgmPausedByVisibility = true;
      [state.normalBgm,state.swordDanceBgm,state.darkSwordSaintBgm,state.bossBgm,state.darkSwordSaintVoice].forEach(a=>{ try{ if(a && !a.paused) a.pause(); }catch(e){} });
    });
  }
...
/* ver.0.5.3: battle log scroll-position preserve + lower padding fix */
(function(){
  'use strict';
  const BUILD=(window.APP_VERSION || window.GAME_VERSION || '0.0');
  const $=(id)=>document.getElementById(id);
...
/* ver.0.5.3: strong/named variants, drop source, quality/luck bonuses, deathdance count-only reset補強 */
(function(){
  'use strict';
  const APP_VERSION = '0.5.3';
  const PREFIXES = ['狡猾な','獰猛な','蛮勇な','エリート','頂点の','原点の','キラー'];
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH 0.5.3]', e); } }
...
/* ver.0.5.3: 敵防御を軽減式へ変更、敵レベル経験値制御、Lv100までの戦闘テンポ調整 */
(function(){
  'use strict';
  const APP_VERSION = '0.5.3';
...
/* ver.0.5.3: 敵Lv経験値完全固定 + 敵攻撃を防御軽減式へ変更 */
(function(){
  'use strict';
  const APP_VERSION = '0.5.3';
  ...
  // 敵ステータス再調整：HPは0.5.3方針維持、攻撃力は防御装備が意味を持つ範囲まで上昇
...
/* ver.0.5.3: 暗黒剣聖の独立Lv表示 + 主人公必要経験値固定 */
(function(){
  'use strict';
  const APP_VERSION = '0.5.3';
...
/* ver.0.5.3 hotfix: hero debug stats, dark equipment list visibility, low-level enemy HP trim */
(function(){
  'use strict';
  const APP_VERSION = '0.5.3';
  function safe(fn){ try{return fn();}catch(e){ console.error('[MBH 0.5.3 hotfix]', e); } }
  function esc(s){ return (typeof escapeHtml === 'function') ? escapeHtml(s) : String(s).replace(/[&<>"']/g, ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])); }
  function pct(v){ return Math.round((Number(v)||0)*100)+'%'; }
...
(function(){
  ...
  function safe(fn){ try{ return fn(); }catch(e){ console.warn('[MBH 0.5.3]', e); return null; } }
  function pct(v){ return Math.round((Number(v)||0)*100) + '%'; }
  function num(v){ return Math.floor(Number(v)||0).toLocaleString(); }
...
/* ver.0.5.3: final XP single-source correction */
(function(){
  'use strict';
  const APP_VERSION = '0.5.3';
  const HERO_XP_NEXT = 1000;
  function safe(fn){ try{ return fn && fn(); }catch(e){ console.warn('[MBH 0.5.3 final]', e); } }
...
/* ver.0.5.3: variant buff list, named regen position, legal links only inside menu, boss low-level tuning, BGM accidental-stop guard */
(function(){
  'use strict';
  const BUILD = (window.APP_VERSION || window.GAME_VERSION || '0.5.3');
  function safe(fn){ try{return fn && fn();}catch(e){ console.error('[MBH0.5.3]', e); return null; } }
  ...
  // 0.5.3の中央上表示をやめ、敵カードの左外へ出す専用クラスにする。
...
/* ver.0.5.3: enemy missing watchdog + debug enemy level/variant restore */
(function(){
  'use strict';
  const BUILD = '0.5.3';
...
/* ver.0.5.3: startup-safe fixed menu layout, footer, log type, debug enemy controls */
(function(){
  'use strict';
  const BUILD='0.5.3';
  ...
  const byId=(id)=>document.getElementById(id);
  function safe(fn){ try{ return fn && fn(); }catch(e){ console.error('[MBH '+BUILD+']', e); return null; } }
  function esc(s){ return String(s ?? '').replace(/[&<>'"]/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c])); }
  function pct(v){ return Math.round((Number(v)||0)*100)+'%'; }
  function num(v){ return Math.floor(Number(v)||0).toLocaleString(); }
  function syncVersion031(){
    safe(()=>{ window.APP_VERSION=BUILD; window.GAME_VERSION=BUILD; document.documentElement.dataset.buildVersion=BUILD; });
    qsa('.build-version,[data-version],#versionText,.version-badge').forEach(el=>{ el.textContent='ver.'+BUILD; });
  ...
  function label(k){
    return ({hp:'HP',atk:'攻撃力',def:'防御力',fireRes:'火軽減',fireDamageHeal:'火吸収',fireDmg:'火ダメージ',fireSkillChance:'炎斬り発動率',thunderDmg:'雷ダメージ',thunderSkillChance:'雷撃発動率',deathDanceChance:'死線の剣舞率',deathDanceDefIgnore:'剣舞防御無視',heroDarkBleedChance:'暗黒出血付与',lifeSteal:'吸収',guard:'ガード率',crit:'会心率',masterRegen:'HP自動回復',darkShield:'闇の盾',darkAmulet:'闇のアミュレット',killHeal:'撃破時HP回復'}[k]||k);
  }
  function rowsHtml(rows, empty){
    if(!rows || !rows.length) return '<div class="effect-empty">'+esc(empty||'なし')+'</div>';
    return rows.map(r=>'<div class="effect-row"><span>'+esc(r[0])+'</span><b>'+esc(r[1])+'</b></div>').join('');
  }
  function statRows031(){
    if(typeof calcStats!=='function') return [];
    const st=calcStats(), rows=[];
    const addP=(k,v,plus=false)=>{ if(v) rows.push([label(k),(plus?'+':'')+pct(v)]); };
    addP('fireRes',st.fireRes); addP('fireDamageHeal',st.fireDamageHeal); addP('fireDmg',st.fireDmg,true); addP('fireSkillChance',st.fireSkillChance,true);
    addP('thunderDmg',st.thunderDmg,true); addP('thunderSkillChance',st.thunderSkillChance,true);
    addP('deathDanceChance',st.deathDanceChance); addP('deathDanceDefIgnore',st.deathDanceDefIgnore); addP('heroDarkBleedChance',st.heroDarkBleedChance);
    addP('lifeSteal',st.lifeSteal); addP('guard',st.guard); addP('crit',st.crit);
    if(st.masterRegen) rows.push([label('masterRegen'), ((st.masterRegenRate||0)*100).toFixed(1)+'% / 10秒']);
    if(st.darkShield) rows.push([label('darkShield'),'有効']);
    if(st.darkAmulet) rows.push([label('darkAmulet'),'有効']);
    return rows;
  }
  function equipRows031(){
    const t={hp:0,atk:0,def:0,fireRes:0,fireDamageHeal:0,fireDmg:0,fireSkillChance:0,thunderDmg:0,thunderSkillChance:0,deathDanceChance:0,deathDanceDefIgnore:0,heroDarkBleedChance:0,lifeSteal:0,guard:0,crit:0,darkShield:false,darkAmulet:false,masterRegen:false};
    if(typeof state!=='undefined' && state.equip){ Object.values(state.equip).filter(Boolean).forEach(it=>{
      Object.keys(t).forEach(k=>{ if(typeof t[k]==='number') t[k]+=Number(it[k])||0; });
      if(it.darkShield) t.darkShield=true; if(it.darkAmulet) t.darkAmulet=true; if(it.masterRegen) t.masterRegen=true;
    }); }
    const rows=[]; const addN=(k,v)=>{ if(v) rows.push([label(k),(v>0?'+':'')+num(v)]); }; const addP=(k,v,plus=true)=>{ if(v) rows.push([label(k),(plus?'+':'')+pct(v)]); };
    addN('hp',t.hp); addN('atk',t.atk); addN('def',t.def);
    addP('fireRes',t.fireRes,false); addP('fireDamageHeal',t.fireDamageHeal,false); addP('fireDmg',t.fireDmg); addP('fireSkillChance',t.fireSkillChance);
    addP('thunderDmg',t.thunderDmg); addP('thunderSkillChance',t.thunderSkillChance); addP('deathDanceChance',t.deathDanceChance);
    addP('deathDanceDefIgnore',t.deathDanceDefIgnore,false); addP('heroDarkBleedChance',t.heroDarkBleedChance,false); addP('lifeSteal',t.lifeSteal,false); addP('guard',t.guard); addP('crit',t.crit);
    if(t.darkShield) rows.push([label('darkShield'),'有効']); if(t.darkAmulet) rows.push([label('darkAmulet'),'有効']); if(t.masterRegen) rows.push([label('masterRegen'),'有効']);
    return rows;
  }
  function removeEnemyXpRows031(){
    const grid=qs('.hero-stats .stat-grid'); if(!grid) return;
    const nodes=Array.from(grid.children);
    for(let i=0;i<nodes.length;i++){
      if(/敵Lv経験値|敵Lv経験|敵レベル経験値/.test((nodes[i].textContent||'').trim())){ const a=nodes[i], b=nodes[i+1]; a.remove(); if(b) b.remove(); }
    }
  }
  function ensureFooter031(){
    const panel=qs('.side-panel'); if(!panel) return;
    qsa('#fixedLegalLinks,.fixed-legal-links').forEach(x=>x.remove());
    let legal=qs('.side-panel > .menu-footer') || qs('.side-panel > .legal-links') || qs('.legal-links');
    if(!legal){
    ...
    qsa('.panel .legal-links,.panel .menu-footer').forEach(x=>{ if(x!==legal) x.remove(); });
    const bind=(id,type)=>{ const b=byId(id); if(!b || b.__mbh031Legal) return; b.__mbh031Legal=true; b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal(type); }); }; };
    bind('creditBtn','credit'); bind('termsBtn','terms'); bind('privacyBtn','privacy');
  }
  function ensureStatusLayout031(){
    const hero=qs('.hero-stats'); if(!hero) return;
    removeEnemyXpRows031();
    let layout=qs(':scope > .status-layout', hero);
    let left=layout && qs(':scope > .status-left', layout);
    let right=layout && qs(':scope > .status-right', layout);
    if(!layout){ layout=document.createElement('div'); layout.className='status-layout'; left=document.createElement('div'); right=document.createElement('div'); left.className='status-left'; right.className='status-right'; layout.append(left,right); hero.appendChild(layout); }
    const h2=qs(':scope > h2', hero);
    const grid=qs('.stat-grid', hero);
    let special=qs('#statusSpecialEffects', hero) || qs('.status-special-effects', hero);
    ...
    const records=qs('.monster-records', hero);
    ...
    qsa('.hero-stats #statusSpecialEffects,.hero-stats .status-special-effects').filter((x,i,a)=>x!==special).forEach(x=>x.remove());
    const box=qs('#statusSpecialEffects .effect-scroll', hero); if(box) box.innerHTML=rowsHtml(statRows031(),'特殊効果なし');
  }
  function ensureEquipLayout031(){
    const equip=qs('.equip-panel'); if(!equip) return;
    let layout=qs(':scope > .equip-layout', equip);
    let left=layout && qs(':scope > .equip-left', layout);
    let right=layout && qs(':scope > .equip-right', layout);
    if(!layout){ layout=document.createElement('div'); layout.className='equip-layout'; left=document.createElement('div'); right=document.createElement('div'); left.className='equip-left'; right.className='equip-right'; layout.append(left,right); equip.appendChild(layout); }
    const h2=qs(':scope > h2', equip);
    const list=byId('equipList'); const up=byId('upgradeBtn');
    let totals=byId('equipEffectTotals') || qs('.equip-effect-totals', equip);
    ...
    if(up && up.parentElement!==left) left.appendChild(up);
    if(totals.parentElement!==right) right.appendChild(totals);
    qsa('.equip-panel #equipEffectTotals,.equip-panel .equip-effect-totals').filter(x=>x!==totals).forEach(x=>x.remove());
    const box=qs('#equipEffectTotals .effect-scroll', equip); if(box) box.innerHTML=rowsHtml(equipRows031(),'装備効果なし');
  }
  function ensureLayouts031(){ syncVersion031(); ensureFooter031(); ensureStatusLayout031(); ensureEquipLayout031(); }
  function classifyLog031(msg, cls){
    const c=String(cls||'').toLowerCase(); const raw=String(msg||''); const m=raw.replace(/<[^>]*>/g,'');
    if(c.includes('drop') || /装備ドロップ|をドロップ|ドロップ元|枠目|闇装備確定/.test(m)) return 'drop';
    if(c.includes('damage') || c.includes('skilllog') || c.includes('danger') || /火属性を吸収|吸収 \+|出血を付与|出血した|暗黒出血|火傷|回復|ダメージ|攻撃|斬|雷撃|炎斬り|連続攻撃|剣舞|ブレス|GUARD|反射|無効|MISS|酸ボディ/.test(m)) return 'damage';
    return 'system';
  }
  function renderLog031(reason='append'){
    if(typeof state==='undefined') return; const el=byId('log'); if(!el) return;
    const f=state.logFilter||'all'; const beforeTop=el.scrollTop, beforeHeight=el.scrollHeight, nearTop=beforeTop<=4;
    const rows=(state.log||[]).filter(l=>f==='all'||(l.type||classifyLog031(l.msg,l.cls))===f);
    el.innerHTML=rows.map(l=>'<div class="'+esc(l.cls||'')+'" data-log-type="'+esc(l.type||classifyLog031(l.msg,l.cls))+'">['+esc(l.time||'')+'] '+(l.msg||'')+'</div>').join('');
    qsa('#logFilterBar button').forEach(b=>b.classList.toggle('active',(b.dataset.logFilter||'all')===f));
    requestAnimationFrame(()=>{ if(reason==='filter') el.scrollTop=0; else el.scrollTop=nearTop?0:Math.max(0,beforeTop+(el.scrollHeight-beforeHeight)); });
  }
  function installLog031(){
    if(typeof state==='undefined' || window.__mbh031LogInstalled) return; window.__mbh031LogInstalled=true; state.logFilter=state.logFilter||'all';
    const bar=byId('logFilterBar'); if(bar){ bar.innerHTML='<button type="button" data-log-filter="all">すべて</button><button type="button" data-log-filter="damage">ダメージ</button><button type="button" data-log-filter="system">システム</button><button type="button" data-log-filter="drop">ドロップ</button>'; qsa('button[data-log-filter]',bar).forEach(b=>{ const on=(e)=>{ e.preventDefault(); e.stopPropagation(); state.logFilter=b.dataset.logFilter||'all'; renderLog031('filter'); return false; }; b.onclick=on; b.ontouchend=on; b.onpointerup=(e)=>{ if(e.pointerType==='mouse') return; return on(e); }; }); }
    log=function(msg, cls='', html=false){ const time=new Date().toLocaleTimeString('ja-JP',{hour12:false}); const safeMsg=html?String(msg):esc(msg); const type=classifyLog031(safeMsg,cls); state.log.unshift({time,msg:safeMsg,cls,html:true,type}); state.log=state.log.slice(0,220); renderLog031('append'); };
    (state.log||[]).forEach(l=>{ l.type=l.type||classifyLog031(l.msg,l.cls); }); renderLog031('filter');
  }
  function installDebug031(){
    const p=byId('debugPanel'); if(!p || p.__mbh031Debug) return; p.__mbh031Debug=true;
    ['debugHp0Kill','debugForceDefeated','debugClearBleed','debugStopDot','debugStopDarkDance'].forEach(id=>safe(()=>byId(id)?.remove()));
    const close=byId('debugClose');
    const add=(id,txt,fn)=>{ let b=byId(id); if(!b){ b=document.createElement('button'); b.id=id; b.type='button'; b.textContent=txt; p.insertBefore(b,close||null); } b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); fn(); }; };
    const adjust=(d)=>{ if(typeof state==='undefined') return; const cur=Math.max(1,Math.floor(Number(state.enemyLevelBase)||Number(state.enemy?.level)||Number(state.level)||1)); state.enemyLevelBase=Math.max(1,cur+d); state.enemyLevelXp=0; state.enemyLevelBaseDefeated=Math.max(0,Math.floor(Number(state.defeated)||0)); safe(()=>log('デバッグ：敵出現レベルをLv.'+cur+'→Lv.'+state.enemyLevelBase+'に変更。','system')); safe(()=>{ if(typeof renderAll==='function') renderAll(); }); safe(()=>{ if(typeof scheduleSave==='function') scheduleSave(); }); };
    add('debugNextStrong','次の敵を強個体',()=>{ state.debugNextVariant031='strong'; log('デバッグ：次の敵を強個体に設定。','system'); });
    add('debugNextNamed','次の敵を異名持ち',()=>{ state.debugNextVariant031='named'; log('デバッグ：次の敵を異名持ちに設定。','system'); });
    add('debugEnemyLvMinus10','敵Lv -10',()=>adjust(-10)); add('debugEnemyLvMinus5','敵Lv -5',()=>adjust(-5)); add('debugEnemyLvPlus5','敵Lv +5',()=>adjust(5)); add('debugEnemyLvPlus10','敵Lv +10',()=>adjust(10));
  }
  function installVariant031(){
    if(typeof makeScaledEnemy!=='function' || window.__mbhMakeScaledEnemy031) return; window.__mbhMakeScaledEnemy031=makeScaledEnemy;
    makeScaledEnemy=function(base, forceLevel=null){ const e=window.__mbhMakeScaledEnemy031.apply(this,arguments); safe(()=>{ if(!e || e.id==='dark_sword_saint' || !state?.debugNextVariant031) return; const type=state.debugNextVariant031; state.debugNextVariant031=null; e.variant={type,label:type==='named'?'異名持ち':'強個体'}; e.name=(type==='named'?'異名の':'強個体 ')+(e.baseName||e.name); e.maxHp=Math.max(1,Math.floor(e.maxHp*(type==='named'?2.0:1.5))); e.atk=Math.max(1,Math.floor(e.atk*(type==='named'?1.25:1.1))); }); return e; };
  }
  function installMenu031(){
    const btn=byId('equipToggleBtn'); const panel=qs('.side-panel'); if(!btn||!panel||btn.__mbh031Menu) return; btn.__mbh031Menu=true;
    const compact=()=>window.matchMedia('(max-width:1279px), (max-height:700px)').matches;
    function sync(){ if(compact()){ btn.style.display='inline-block'; btn.textContent=panel.classList.contains('open')?'閉じる':'メニュー'; } else { btn.style.display='none'; btn.textContent='メニュー'; panel.classList.remove('open'); } }
    btn.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); if(!compact()) return false; panel.classList.toggle('open'); sync(); ensureLayouts031(); return false; };
    window.addEventListener('resize',()=>setTimeout(()=>{ sync(); ensureLayouts031(); },30)); sync();
  }
  function installCss031(){
    if(byId('mbh031-css')) return; const st=document.createElement('style'); st.id='mbh031-css';
    st.textContent=`
      .fixed-legal-links{display:none!important;}
      .side-panel{min-height:0!important;min-width:0!important;overflow:hidden!important;}
      .side-panel{display:flex!important;flex-direction:column!important;gap:10px!important;}
      .side-panel .mobile-menu-tabs{flex:0 0 auto!important;display:flex!important;gap:6px!important;width:100%!important;}
      .side-panel .mobile-menu-tabs button{flex:1 1 0!important;min-width:0!important;}
      .side-panel .panel{min-height:0!important;min-width:0!important;overflow:hidden!important;}
      .side-panel .panel:not(.active-page){display:none!important;}
      .side-panel .panel.active-page{display:flex!important;flex-direction:column!important;flex:1 1 auto!important;}
      .menu-col,.menu-left,.menu-right{display:contents!important;}
      .side-panel>.legal-links,.menu-footer{flex:0 0 auto!important;display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:6px!important;position:static!important;width:100%!important;padding:8px!important;margin:0!important;border:1px solid #73531f!important;border-radius:6px!important;background:linear-gradient(rgba(20,14,8,.96),rgba(5,5,5,.96))!important;z-index:auto!important;box-shadow:none!important;}
      .status-layout,.equip-layout{flex:1 1 auto!important;min-height:0!important;display:grid!important;grid-template-columns:minmax(0,1fr) minmax(280px,.9fr)!important;gap:12px!important;overflow:hidden!important;}
      .status-left,.status-right,.equip-left,.equip-right{min-width:0!important;min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      .status-special-effects,.equip-effect-totals,.monster-records{min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      .status-special-effects{flex:1 1 auto!important;margin-top:10px!important;border-top:1px solid #513917!important;padding-top:8px!important;}
      .monster-records,.equip-effect-totals{flex:1 1 auto!important;margin-top:0!important;border-left:1px solid #513917!important;padding-left:10px!important;}
      .effect-scroll,.monster-record-list{flex:1 1 auto!important;min-height:0!important;max-height:none!important;overflow-y:auto!important;overflow-x:hidden!important;padding-right:8px!important;}
      .equip-left .equip-list{flex:1 1 auto!important;min-height:0!important;overflow-y:auto!important;align-content:start!important;}
      @media(min-width:1280px) and (min-height:701px){.layout{grid-template-columns:minmax(820px,1fr) minmax(620px,760px)!important;}#equipToggleBtn{display:none!important}.side-panel{display:flex!important;position:static!important;width:auto!important;background:none!important;border:0!important;box-shadow:none!important;padding:0!important;}.side-panel>.menu-footer,.side-panel>.legal-links{display:grid!important;}}
      @media(max-width:1279px),(max-height:700px){#equipToggleBtn{display:inline-block!important}.layout{grid-template-columns:1fr!important}.side-panel{display:none!important}.side-panel.open{display:flex!important;position:fixed!important;left:8px!important;right:8px!important;top:calc(var(--mbh-topbar-h,58px) + 8px)!important;bottom:8px!important;width:auto!important;max-width:calc(100vw - 16px)!important;z-index:70!important;background:rgba(0,0,0,.94)!important;padding:8px!important;border:2px solid #c99b39!important;box-shadow:0 0 35px #000!important;}}
      @media(max-width:760px){.status-layout,.equip-layout{grid-template-columns:1fr!important;overflow-y:auto!important}.monster-records,.equip-effect-totals{border-left:0!important;padding-left:0!important;border-top:1px solid #513917!important;padding-top:8px!important;margin-top:8px!important}.menu-footer{grid-template-columns:1fr!important;}}
    `;
    document.head.appendChild(st);
  }
  function patchRender031(){
    if(typeof renderStats==='function' && !window.__mbhRenderStats031){ window.__mbhRenderStats031=renderStats; renderStats=function(){ const r=window.__mbhRenderStats031.apply(this,arguments); ensureStatusLayout031(); return r; }; }
    if(typeof renderEquip==='function' && !window.__mbhRenderEquip031){ window.__mbhRenderEquip031=renderEquip; renderEquip=function(){ const r=window.__mbhRenderEquip031.apply(this,arguments); ensureEquipLayout031(); return r; }; }
    if(typeof renderAll==='function' && !window.__mbhRenderAll031){ window.__mbhRenderAll031=renderAll; renderAll=function(){ const r=window.__mbhRenderAll031.apply(this,arguments); ensureLayouts031(); return r; }; }
  }
  function boot031(){ installCss031(); syncVersion031(); installLog031(); installDebug031(); installVariant031(); patchRender031(); ensureLayouts031(); installMenu031(); }
  setInterval(()=>safe(()=>{ syncVersion031(); ensureFooter031(); removeEnemyXpRows031(); }), 650);
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot031, {once:true}); else setTimeout(boot031,0);
  window.addEventListener('load', boot031, {once:true});
...
/* ver.0.5.3: root cache-bust, fixed split layouts, fixed footer, fixed log categories */
(function(){
  'use strict';
  const BUILD = '0.5.3';
  ...
  const byId = (id)=>document.getElementById(id);
  const safe = (fn)=>{ try{return fn();}catch(e){ console.error('[MBH 0.5.3]', e); } };
  ...
  function removeBadDebug032(){
    ['debugHp0Kill','debugForceDefeated','debugClearBleed','debugStopDot','debugStopDarkDance'].forEach(id=>safe(()=>byId(id)?.remove()));
  }
  function statRows032(){
    const rows=[];
    const add=(k,v)=>{ if(v!==undefined && v!==null && v!==0 && v!=='0%' && v!=='0') rows.push([k,v]); };
    let st={}; safe(()=>{ if(typeof calcStats==='function') st=calcStats()||{}; });
    const pct=(v,plus=false)=> (plus && Number(v)>0?'+':'') + Math.round(Number(v||0)*100)+'%';
    add('火軽減', pct(st.fireRes,false));
    add('火吸収', pct(st.fireDamageHeal,false));
    add('雷ダメージ', pct(st.thunderDmg,true));
    add('雷撃発動率', pct(st.thunderSkillChance,true));
    add('死線の剣舞率', Math.round(Number(st.deathDanceChance||0)*100)+'%');
    add('HP吸収', pct(st.lifeSteal,false));
    add('ガード率', pct(st.guard,true));
    add('クリティカル率', pct(st.crit,true));
    add('経験値獲得量', pct(st.xpGain||st.expGain||0,true));
    add('アイテムドロップ率', pct(st.dropRate||0,true));
    return rows;
  }
  function equipRows032(){
    const rows=[]; const totals={};
    const labelMap={hp:'HP',atk:'攻撃力',def:'防御力',fireRes:'火軽減',fireDamageHeal:'火吸収',fireDmg:'火ダメージ',fireSkillChance:'炎撃発動率',thunderDmg:'雷ダメージ',thunderSkillChance:'雷撃発動率',deathDanceChance:'死線の剣舞率',deathDanceDefIgnore:'剣舞防御無視',heroDarkBleedChance:'暗黒出血',lifeSteal:'HP吸収',guard:'ガード率',crit:'クリティカル率'};
    ...
    safe(()=>{ Object.values((typeof state!=='undefined' && state.equip)||{}).filter(Boolean).forEach(it=>{ Object.keys(labelMap).forEach(k=>{ if(it[k]) totals[k]=(totals[k]||0)+Number(it[k]||0); }); if(it.darkShield) totals.darkShield=1; if(it.darkAmulet) totals.darkAmulet=1; if(it.masterRegen) totals.masterRegen=1; }); });
    Object.keys(labelMap).forEach(k=>{ const v=totals[k]; if(!v) return; rows.push([labelMap[k], pctKeys.has(k) ? ((v>0 && !['fireRes','fireDamageHeal','lifeSteal','deathDanceDefIgnore','heroDarkBleedChance'].includes(k)?'+':'')+Math.round(v*100)+'%') : ((v>0?'+':'')+Math.floor(v).toLocaleString())]); });
  ...
  function removeEnemyXpRows032(){
    const grid=$('.hero-stats .stat-grid'); if(!grid) return;
    const nodes=Array.from(grid.children);
    for(let i=0;i<nodes.length;i++){
      if(/敵Lv経験値|敵Lv経験|敵レベル経験値/.test((nodes[i].textContent||'').trim())){ const a=nodes[i], b=nodes[i+1]; a.remove(); if(b) b.remove(); }
    }
  }
  function ensureFooter032(){
    const panel=$('.side-panel'); if(!panel) return;
    $$('#fixedLegalLinks,.fixed-legal-links').forEach(x=>x.remove());
    let legal=$(':scope > .legal-links.menu-footer', panel) || $(':scope > .legal-links', panel) || $('.legal-links');
    if(!legal){
      legal=document.createElement('div'); legal.innerHTML='<button id="creditBtn" type="button">クレジット</button><button id="termsBtn" type="button">利用規約</button><button id="privacyBtn" type="button">プライバシーポリシー</button>';
    }
    legal.className='legal-links menu-footer'; legal.setAttribute('aria-label','公開情報');
    if(legal.parentElement!==panel) panel.appendChild(legal); else panel.appendChild(legal);
    $$('.panel .legal-links,.panel .menu-footer').forEach(x=>{ if(x!==legal) x.remove(); });
    const bind=(id,type)=>{ const b=byId(id); if(!b) return; b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal(type); }); }; };
    bind('creditBtn','credit'); bind('termsBtn','terms'); bind('privacyBtn','privacy');
  }
  function ensureStatusLayout032(){
    const hero=$('.hero-stats'); if(!hero) return; removeEnemyXpRows032();
    let layout=$(':scope > .status-layout', hero); let left=layout&&$('.status-left',layout); let right=layout&&$('.status-right',layout);
    if(!layout){ layout=document.createElement('div'); layout.className='status-layout'; left=document.createElement('div'); right=document.createElement('div'); left.className='status-left'; right.className='status-right'; layout.append(left,right); hero.appendChild(layout); }
    const grid=$('.stat-grid', hero); let special=byId('statusSpecialEffects') || $('.status-special-effects',hero); const records=$('.monster-records', hero);
    if(!special){ special=document.createElement('div'); special.id='statusSpecialEffects'; special.className='status-special-effects'; special.innerHTML='<h3>特殊効果</h3><div class="effect-scroll"></div>'; }
    if(grid && grid.parentElement!==left) left.appendChild(grid);
    if(special.parentElement!==left) left.appendChild(special);
    if(records && records.parentElement!==right) right.appendChild(records);
    $$('.hero-stats .status-special-effects,.hero-stats #statusSpecialEffects').forEach(x=>{ if(x!==special) x.remove(); });
    const box=$('#statusSpecialEffects .effect-scroll', hero); if(box) box.innerHTML=rowsHtml(statRows032(),'特殊効果なし');
  }
  function ensureEquipLayout032(){
    ...
    if(!layout){ layout=document.createElement('div'); layout.className='equip-layout'; left=document.createElement('div'); right=document.createElement('div'); left.className='equip-left'; right.className='equip-right'; layout.append(left,right); equip.appendChild(layout); }
    const list=byId('equipList'); const up=byId('upgradeBtn'); let totals=byId('equipEffectTotals') || $('.equip-effect-totals',equip);
    if(!totals){ totals=document.createElement('div'); totals.id='equipEffectTotals'; totals.className='equip-effect-totals'; totals.innerHTML='<h3>装備効果合計</h3><div class="effect-scroll"></div>'; }
    if(list && list.parentElement!==left) left.appendChild(list);
    if(up && up.parentElement!==left) left.appendChild(up);
    if(totals.parentElement!==right) right.appendChild(totals);
    $$('.equip-panel .equip-effect-totals,.equip-panel #equipEffectTotals').forEach(x=>{ if(x!==totals) x.remove(); });
    const box=$('#equipEffectTotals .effect-scroll', equip); if(box) box.innerHTML=rowsHtml(equipRows032(),'装備効果なし');
  }
  function classify032(msg, cls){
    const c=String(cls||'').toLowerCase(); const text=String(msg||'').replace(/<[^>]*>/g,'');
    if(c.includes('drop') || /装備ドロップ|をドロップ|ドロップ元|枠目|闇装備確定|討伐報酬/.test(text)) return 'drop';
    if(c.includes('damage') || c.includes('skilllog') || c.includes('danger') || /火属性を吸収|吸収 \+|出血を付与|出血した|暗黒出血|火傷|回復|ダメージ|攻撃|斬|雷撃|炎斬り|連続攻撃|剣舞|ブレス|GUARD|反射|無効|MISS|酸ボディ/.test(text)) return 'damage';
    return 'system';
  }
  function renderLog032(reason='append'){
    if(typeof state==='undefined') return; const el=byId('log'); if(!el) return;
    const f=state.logFilter||'all'; const beforeTop=el.scrollTop, beforeHeight=el.scrollHeight, nearBottom=(beforeTop+el.clientHeight>=beforeHeight-8);
    const rows=(state.log||[]).filter(l=>f==='all'||(l.type||classify032(l.msg,l.cls))===f);
    el.innerHTML=rows.map(l=>'<div class="'+esc(l.cls||'')+'" data-log-type="'+esc(l.type||classify032(l.msg,l.cls))+'">['+esc(l.time||'')+'] '+(l.msg||'')+'</div>').join('');
  ...
  function installLog032(){
    if(typeof state==='undefined') return; state.logFilter=state.logFilter||'all';
    const bar=byId('logFilterBar'); if(bar && !bar.__mbh032){ bar.__mbh032=true; bar.innerHTML='<button type="button" data-log-filter="all">すべて</button><button type="button" data-log-filter="damage">ダメージ</button><button type="button" data-log-filter="system">システム</button><button type="button" data-log-filter="drop">ドロップ</button>'; $$('button[data-log-filter]',bar).forEach(b=>{ const on=(e)=>{ e.preventDefault(); e.stopPropagation(); state.logFilter=b.dataset.logFilter||'all'; renderLog032('filter'); return false; }; b.onclick=on; b.ontouchend=on; }); }
    if(!window.__mbh032LogInstalled){ window.__mbh032LogInstalled=true; log=function(msg, cls='', html=false){ const time=new Date().toLocaleTimeString('ja-JP',{hour12:false}); const safeMsg=html?String(msg):esc(msg); const type=classify032(safeMsg,cls); state.log.unshift({time,msg:safeMsg,cls,html:true,type}); state.log=state.log.slice(0,220); renderLog032('append'); }; }
    (state.log||[]).forEach(l=>{ l.type=classify032(l.msg,l.cls); }); renderLog032('filter');
  }
  function installDebug032(){
    const p=byId('debugPanel'); if(!p) return; removeBadDebug032(); const close=byId('debugClose');
    const add=(id,txt,fn)=>{ let b=byId(id); if(!b){ b=document.createElement('button'); b.id=id; b.type='button'; b.textContent=txt; p.insertBefore(b,close||null); } b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); fn(); }; };
    const adjust=(d)=>{ if(typeof state==='undefined') return; const cur=Math.max(1,Math.floor(Number(state.enemyLevelBase)||Number(state.enemy?.level)||Number(state.level)||1)); state.enemyLevelBase=Math.max(1,cur+d); state.enemyLevelXp=0; state.enemyLevelBaseDefeated=Math.max(0,Math.floor(Number(state.defeated)||0)); safe(()=>log('デバッグ：敵出現レベルをLv.'+cur+'→Lv.'+state.enemyLevelBase+'に変更。','system')); safe(()=>{ if(typeof renderAll==='function') renderAll(); }); safe(()=>{ if(typeof scheduleSave==='function') scheduleSave(); }); };
    add('debugNextStrong','次の敵を強個体',()=>{ state.debugNextVariant031='strong'; log('デバッグ：次の敵を強個体に設定。','system'); });
    add('debugNextNamed','次の敵を異名持ち',()=>{ state.debugNextVariant031='named'; log('デバッグ：次の敵を異名持ちに設定。','system'); });
  ...
  function installMenu032(){
    const btn=byId('equipToggleBtn'); const panel=$('.side-panel'); if(!btn||!panel) return;
    const compact=()=>window.matchMedia('(max-width:1279px), (max-height:700px)').matches;
    function sync(){ if(compact()){ btn.style.display='inline-block'; btn.textContent=panel.classList.contains('open')?'閉じる':'メニュー'; } else { btn.style.display='none'; btn.textContent='メニュー'; panel.classList.remove('open'); } }
    btn.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); if(!compact()) return false; panel.classList.toggle('open'); sync(); ensureAll032(); return false; };
    window.addEventListener('resize',()=>setTimeout(()=>{ sync(); ensureAll032(); },30)); sync();
  }
  function ensureEnemy032(){
    if(typeof state==='undefined' || typeof spawnEnemy!=='function') return;
    if(!state.enemy && !state.defeatSequence && !state.down){
      if(!window.__mbh032LastEnemyFix || Date.now()-window.__mbh032LastEnemyFix>1500){ window.__mbh032LastEnemyFix=Date.now(); safe(()=>spawnEnemy(false)); }
    }
  }
  function ensureAll032(){ syncVersion032(); ensureFooter032(); ensureStatusLayout032(); ensureEquipLayout032(); installLog032(); installDebug032(); removeBadDebug032(); }
  function boot032(){ ensureAll032(); installMenu032(); ensureEnemy032(); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot032, {once:true}); else setTimeout(boot032,0);
  window.addEventListener('load', boot032, {once:true});
  setInterval(()=>safe(()=>{ ensureAll032(); ensureEnemy032(); }), 1000);
...
/* ver.0.5.3: footer isolated from log redraw + final log type lock */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const qs=(s,r=document)=>r.querySelector(s);
  ...
  function syncVersion033(){
    ...
    qsa('.debug-version').forEach(el=>{ el.textContent='Build: ver.'+BUILD+' debug'; });
    const t=byId('mbhTraceBox')?.querySelector('.debug-trace-title');
    if(t) t.textContent='進行デバッグログ ver.'+BUILD;
  }

  function ensureFooter033(){
    const panel=qs('.side-panel');
    if(!panel) return;
    qsa('#fixedLegalLinks,.fixed-legal-links').forEach(x=>x.remove());
    qsa('.side-panel .panel .legal-links,.side-panel .panel .menu-footer').forEach(x=>x.remove());
    let legal=qs(':scope > .legal-links.menu-footer', panel) || qs(':scope > .legal-links', panel);
    if(!legal){
    ...
    const bind=(id,type)=>{
      ...
      b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal(type); }); return false; };
    };
    bind('creditBtn','credit'); bind('termsBtn','terms'); bind('privacyBtn','privacy');
  }

  function classifyLog033(msg, cls){
    const c=String(cls||'').toLowerCase();
    const text=String(msg||'').replace(/<[^>]*>/g,'');
    // ダメージ扱いを最優先。古いdrop/systemタグが付いていてもここで上書きする。
    if(/火属性を吸収|吸収 \+|出血を付与|出血した|暗黒出血|火傷|被ダメ回復|HP[^\n]*回復|回復|ダメージ|攻撃|斬|雷撃|炎斬り|連続攻撃|剣舞|ブレス|GUARD|反射|無効|MISS|酸ボディ/.test(text)) return 'damage';
    if(c.includes('damage') || c.includes('skilllog') || c.includes('danger')) return 'damage';
    if(/装備ドロップ|をドロップ|ドロップ元|枠目|闇装備確定|討伐報酬|獲得装備/.test(text)) return 'drop';
    if(c.includes('drop')) return 'drop';
    return 'system';
  }

  function normalizeAllLogTypes033(){
    if(typeof state==='undefined' || !Array.isArray(state.log)) return;
    state.log.forEach(l=>{ l.type=classifyLog033(l.msg,l.cls); });
  }

  function renderLog033(reason='append'){
    if(typeof state==='undefined') return;
    const el=byId('log'); if(!el) return;
    normalizeAllLogTypes033();
    const f=state.logFilter||'all';
    const beforeTop=el.scrollTop;
    const beforeHeight=el.scrollHeight;
    const nearBottom=beforeTop+el.clientHeight>=beforeHeight-8;
  ...
  function installLog033(){
    ...
    if(bar && !bar.__mbh033){
      bar.__mbh033=true;
      bar.innerHTML='<button type="button" data-log-filter="all">すべて</button><button type="button" data-log-filter="damage">ダメージ</button><button type="button" data-log-filter="system">システム</button><button type="button" data-log-filter="drop">ドロップ</button>';
      qsa('button[data-log-filter]',bar).forEach(b=>{
        const on=(e)=>{ e.preventDefault(); e.stopPropagation(); state.logFilter=b.dataset.logFilter||'all'; renderLog033('filter'); return false; };
        b.onclick=on; b.ontouchend=on; b.onpointerup=(e)=>{ if(e.pointerType==='mouse') return; return on(e); };
      });
    }
    if(!window.__mbh033LogInstalled){
      window.__mbh033LogInstalled=true;
      log=function(msg, cls='', html=false){
        const time=new Date().toLocaleTimeString('ja-JP',{hour12:false});
        const safeMsg=html?String(msg):esc(msg);
        const type=classifyLog033(safeMsg,cls);
  ...
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot033, {once:true}); else setTimeout(boot033,0);
  window.addEventListener('load', boot033, {once:true});
  // 古いパッチが1秒ごとにフッターやログ分類を戻すため、最終固定だけを軽く再適用する。
  setInterval(()=>safe(()=>{ syncVersion033(); ensureFooter033(); installLog033(); normalizeAllLogTypes033(); }), 500);
  safe(()=>{ window.mbh033Check=()=>({build:document.documentElement.dataset.buildVersion, footerParent:qs('.menu-footer')?.parentElement?.className||'', footerInPanel:!!qs('.panel .legal-links,.panel .menu-footer'), filter:typeof state!=='undefined'?state.logFilter:null, badLogTypes:typeof state!=='undefined'?(state.log||[]).filter(l=>/火属性を吸収|出血を付与|出血した/.test(String(l.msg||'')) && l.type!=='damage').slice(0,5):[]}); });
})();


/* ver.0.5.3: boss BGM + startup menu closed final lock */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const qs=(s,r=document)=>r.querySelector(s);
  ...
  function forceMenuClosedOnBoot(){
    const side=qs('.side-panel'); const btn=document.getElementById('equipToggleBtn');
    if(!side) return;
    if(!window.__mbh034MenuUserTouched){
      side.classList.remove('open');
      safe(()=>{ if(typeof state!=='undefined') state.uiOpen=false; });
    }
    if(btn){
      btn.textContent = (compact() && side.classList.contains('open')) ? '閉じる' : 'メニュー';
      btn.style.display = compact() ? 'inline-block' : 'none';
    }
  }
  function installMenu034(){
    const side=qs('.side-panel'); const btn=document.getElementById('equipToggleBtn');
    if(!side||!btn||btn.__mbh034) return; btn.__mbh034=true;
    const toggle=(e)=>{
      e.preventDefault(); e.stopPropagation();
      window.__mbh034MenuUserTouched=true;
      if(!compact()){
        side.classList.remove('open');
        safe(()=>{ if(typeof state!=='undefined') state.uiOpen=false; });
      ...
      side.classList.toggle('open', open);
      safe(()=>{ if(typeof state!=='undefined') state.uiOpen=open; });
      btn.textContent=open?'閉じる':'メニュー';
      safe(()=>{ if(typeof setMenuPage==='function' && typeof state!=='undefined') setMenuPage(state.menuPage||'stats'); });
      safe(()=>{ if(typeof ensureAll032==='function') ensureAll032(); });
      safe(()=>{ if(typeof startAudio==='function') startAudio(); });
      safe(()=>{ if(typeof playUiClick==='function') playUiClick(); });
      return false;
    };
    ...
    window.addEventListener('resize', ()=>setTimeout(forceMenuClosedOnBoot, 30));
  }
  function installBossBgm034(){
    safe(()=>{
      if(typeof ensureBgmAudio==='function' && !window.__mbhEnsureBgmAudio034){
        window.__mbhEnsureBgmAudio034=ensureBgmAudio;
        ensureBgmAudio=function(){
          const r=window.__mbhEnsureBgmAudio034.apply(this, arguments);
          if(typeof state!=='undefined' && !state.bossBgm){
            state.bossBgm=new Audio('Before_the_Gate_Falls.mp3');
            state.bossBgm.loop=true; state.bossBgm.preload='auto';
          }
          if(typeof updateBgmVolume==='function') updateBgmVolume();
          return r;
        };
      }
      if(typeof updateBgmVolume==='function' && !window.__mbhUpdateBgmVolume034){
        window.__mbhUpdateBgmVolume034=updateBgmVolume;
        updateBgmVolume=function(){
          const r=window.__mbhUpdateBgmVolume034.apply(this, arguments);
          if(typeof state!=='undefined' && state.bossBgm){ state.bossBgm.volume = state.mobileMuted ? 0 : Math.max(0, Math.min(2, state.volume)) * 0.05; }
          return r;
        };
      }
      if(typeof setEnemy==='function' && !window.__mbhSetEnemy034){
        window.__mbhSetEnemy034=setEnemy;
        setEnemy=function(e){
          const r=window.__mbhSetEnemy034.apply(this, arguments);
          if(e && e.type==='ボス' && e.id!=='dark_sword_saint') safe(()=>setBgmMode('boss'));
          return r;
        };
      }
    });
  }
  function boot034(){ syncVersion034(); installBossBgm034(); forceMenuClosedOnBoot(); installMenu034(); setTimeout(forceMenuClosedOnBoot,80); setTimeout(forceMenuClosedOnBoot,400); }
...
/* ver.0.5.3: final stable menu/footer/effects/log/sell-exp lock */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(s,r=document)=>r.querySelector(s);
...
/* ver.0.5.3: menu initial-close hard lock.
...
(function(){
  ...
  function installCss037(){
    if(byId('mbh037-menu-lock-css')) return;
    const st=document.createElement('style');
    st.id='mbh037-menu-lock-css';
    st.textContent=`
      @media (max-width:1279px), (max-height:700px), (pointer:coarse){
        body:not(.mbh-menu-open) .side-panel,
        body:not(.mbh-menu-open) .side-panel.open{display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important;}
        body.mbh-menu-open .side-panel,
        body.mbh-menu-open .side-panel.open{display:flex!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;}
        #equipToggleBtn{display:inline-block!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;}
      }
      @media (min-width:1280px) and (min-height:701px) and (pointer:fine){
        body .side-panel,body .side-panel.open{display:flex!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;position:static!important;}
        #equipToggleBtn{display:none!important;}
      }
    `;
    document.head.appendChild(st);
  ...
  function applyMenu037(force){
    if(typeof force==='boolean') menuOpen=force;
    const side=qs('.side-panel');
    const btn=byId('equipToggleBtn');
    if(!side) return;
    if(compact()){
      document.body.classList.toggle('mbh-menu-open', !!menuOpen);
      side.classList.toggle('open', !!menuOpen);
      side.style.display = menuOpen ? 'flex' : 'none';
      side.style.visibility = menuOpen ? 'visible' : 'hidden';
      side.style.opacity = menuOpen ? '1' : '0';
      side.style.pointerEvents = menuOpen ? 'auto' : 'none';
      if(btn){ btn.style.display='inline-block'; btn.textContent='メニュー'; }
      safe(()=>{ if(typeof state!=='undefined') state.uiOpen=!!menuOpen; });
    }else{
      menuOpen=false;
      document.body.classList.remove('mbh-menu-open');
      side.classList.remove('open');
      side.style.display='flex';
      side.style.visibility='visible';
      side.style.opacity='1';
      side.style.pointerEvents='auto';
      if(btn){ btn.style.display='none'; btn.textContent='メニュー'; }
      safe(()=>{ if(typeof state!=='undefined') state.uiOpen=false; });
    }
  }

  function toggleFromUser037(e){
    const target=e.target && e.target.closest ? e.target.closest('#equipToggleBtn') : null;
    if(!target) return;
    e.preventDefault();
    e.stopPropagation();
    if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    const now=Date.now();
    if(now-lastToggle<240) return false;
    lastToggle=now;
    if(compact()) menuOpen=!menuOpen;
    else menuOpen=false;
    applyMenu037(menuOpen);
    safe(()=>{ if(typeof startAudio==='function') startAudio(); });
    safe(()=>{ if(typeof playUiClick==='function') playUiClick(); });
    return false;
  }
  ...
  function bind037(){
    syncVersion037();
    installCss037();
    // 起動時は必ず閉じる。大画面は右固定なのでopenクラス不要。
    menuOpen=false;
    applyMenu037(false);
    ['click','pointerup','touchend'].forEach(type=>{
      document.removeEventListener(type, toggleFromUser037, true);
      document.addEventListener(type, toggleFromUser037, {capture:true, passive:false});
    });
  }

  function start037(){
    bind037();
    setTimeout(()=>{ menuOpen=false; applyMenu037(false); syncVersion037(); }, 60);
    setTimeout(()=>{ menuOpen=false; applyMenu037(false); syncVersion037(); }, 700);
    setTimeout(()=>{ menuOpen=false; applyMenu037(false); syncVersion037(); }, 1400);
  }

  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', start037, {once:true}); else start037();
  window.addEventListener('load', start037, {once:true});
  window.addEventListener('resize', ()=>setTimeout(()=>applyMenu037(menuOpen),40));
  window.addEventListener('orientationchange', ()=>setTimeout(()=>applyMenu037(false),80));

  // Older stacked intervals may add .open again. CSS blocks it, and this keeps class/state clean.
  setInterval(()=>{
    safe(()=>{
      syncVersion037();
      const side=qs('.side-panel');
...
/* ver.0.5.3: final layout/log/effect/menu stability lock */
(function(){
  'use strict';
  const BUILD='0.5.3';
  ...
  function syncVersion038(){
    ...
    const trace=document.querySelector('#mbhTraceBox .debug-trace-title'); if(trace) trace.textContent='進行デバッグログ ver.'+BUILD;
  }

  function side(){ return document.querySelector('.side-panel'); }
  function currentOpen(){ return document.body.classList.contains('mbh-menu-open'); }
  function setMenuOpen038(open){
    const sp=side();
    const btn=$id('equipToggleBtn');
    const should=!!open && compact();
    if(sp){ sp.classList.toggle('open', should); sp.setAttribute('aria-hidden', should?'false':'true'); }
    document.body.classList.toggle('mbh-menu-open', should);
    if(typeof state!=='undefined') state.uiOpen=should;
    if(btn){ btn.textContent=should?'閉じる':'メニュー'; btn.setAttribute('aria-expanded',should?'true':'false'); }
  }

  function installMenuToggle038(){
    const btn=$id('equipToggleBtn'); if(!btn || btn.__mbh038) return;
    btn.__mbh038=true;
    const fresh=btn.cloneNode(true);
    btn.parentNode.replaceChild(fresh,btn);
    fresh.__mbh038=true;
    fresh.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); setMenuOpen038(!currentOpen()); safe(()=>{ if(typeof startAudio==='function') startAudio(); if(typeof playUiClick==='function') playUiClick(); }); return false; };
    fresh.onpointerup=(e)=>{ if(e.pointerType==='mouse') return; return fresh.onclick(e); };
    fresh.ontouchend=fresh.onclick;
  }
  function syncResponsive038(){
    if(!compact()){
      const sp=side(); if(sp){ sp.classList.remove('open'); sp.removeAttribute('aria-hidden'); }
      document.body.classList.remove('mbh-menu-open');
      if(typeof state!=='undefined') state.uiOpen=false;
      const btn=$id('equipToggleBtn'); if(btn){ btn.textContent='メニュー'; btn.setAttribute('aria-expanded','false'); }
    }
  }

  function ensureFooter038(){
    const sp=side(); if(!sp) return;
    let legal=sp.querySelector(':scope > .legal-links.menu-footer') || sp.querySelector(':scope > .legal-links') || document.querySelector('.legal-links');
    if(!legal){
      legal=document.createElement('div'); legal.className='legal-links menu-footer'; legal.setAttribute('aria-label','公開情報');
      legal.innerHTML='<button id="creditBtn" type="button">クレジット</button><button id="termsBtn" type="button">利用規約</button><button id="privacyBtn" type="button">プライバシーポリシー</button>';
    }
    legal.className='legal-links menu-footer'; legal.classList.remove('hidden');
    if(legal.parentElement!==sp) sp.appendChild(legal);
    $$('.panel .legal-links,.panel .menu-footer,#fixedLegalLinks,.fixed-legal-links').forEach(x=>{ if(x!==legal) x.remove(); });
    const bind=(id,type)=>{ const b=$id(id); if(b && !b.__mbh038){ b.__mbh038=true; b.onclick=(e)=>{e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); if(typeof openLegalModal==='function') openLegalModal(type); }); return false;}; } };
    bind('creditBtn','credit'); bind('termsBtn','terms'); bind('privacyBtn','privacy');
  }

  function setMenuPage038(page){
    if(typeof state!=='undefined') state.menuPage=page||state.menuPage||'stats';
    const p=(typeof state!=='undefined' && state.menuPage) || 'stats';
    const map={stats:'.hero-stats',equip:'.equip-panel',inventory:'.inventory-panel',log:'.log-panel'};
    $$('.mobile-menu-tabs button').forEach(b=>b.classList.toggle('active', b.dataset.menuPage===p));
    $$('.side-panel .panel').forEach(panel=>panel.classList.remove('active-page'));
    const target=document.querySelector(map[p]||map.stats); if(target) target.classList.add('active-page');
  }
  function installTabs038(){
    $$('.mobile-menu-tabs button').forEach(b=>{
      if(b.__mbh038) return; b.__mbh038=true;
      b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); setMenuPage038(b.dataset.menuPage||'stats'); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); return false; };
      b.ontouchend=b.onclick;
    });
  }

  function removeEnemyXpLine038(){
    $id('statEnemyLevelXp')?.remove(); $id('statEnemyLevelXpLabel')?.remove();
    $$('.stat-grid span').forEach(sp=>{ if((sp.textContent||'').includes('敵Lv経験値')){ const nxt=sp.nextElementSibling; sp.remove(); if(nxt && nxt.tagName==='B') nxt.remove(); } });
  }

  function labelRowsFromStats(st){
    const rows=[]; const add=(name,val,kind='pct')=>{ if(!val) return; rows.push([name,kind==='pct'?pct(val):String(val)]); };
    add('火軽減',st.fireRes); add('火吸収',st.fireDamageHeal); add('雷ダメージ',st.thunderDmg); add('雷属性強化',st.thunderSkillChance);
    add('死線の剣舞率',st.deathDanceChance); add('HP吸収',st.lifeSteal); add('ガード',st.guard); add('会心',st.crit);
    add('防御無視',st.deathDanceDefIgnore); add('暗黒出血付与',st.heroDarkBleedChance);
    if(st.masterRegen) rows.push(['師匠のアミュレット',`${pct(st.masterRegenRate)} / 10秒`]);
    if(st.darkShield) rows.push(['闇の盾','被ダメ軽減/被ダメ回復']);
    if(st.darkAmulet) rows.push(['闇のアミュレット','剣舞時間2倍']);
    return rows;
  }
  function equipmentTotals038(){
    const t={hp:0,atk:0,def:0,fireRes:0,fireDamageHeal:0,thunderDmg:0,thunderSkillChance:0,deathDanceChance:0,lifeSteal:0,guard:0,crit:0,deathDanceDefIgnore:0,heroDarkBleedChance:0};
    if(typeof state==='undefined') return t;
    Object.values(state.equip||{}).filter(Boolean).forEach(it=>{
      t.hp+=Number(it.hp)||0; t.atk+=Number(it.atk)||0; t.def+=Number(it.def)||0; t.fireRes+=Number(it.fireRes)||0; t.fireDamageHeal+=Number(it.fireDamageHeal)||0;
      t.thunderDmg+=Number(it.thunderDmg)||0; t.thunderSkillChance+=Number(it.thunderSkillChance)||0; t.deathDanceChance+=Number(it.deathDanceChance)||0;
      t.lifeSteal+=Number(it.lifeSteal)||0; t.guard+=Number(it.guard)||0; t.crit+=Number(it.crit)||0; t.deathDanceDefIgnore+=Number(it.deathDanceDefIgnore)||0; t.heroDarkBleedChance+=Number(it.heroDarkBleedChance)||0;
    });
    return t;
  }
  function rowsHtml038(rows,empty){
    if(!rows.length) return `<div class="effect-empty">${esc(empty||'なし')}</div>`;
    return rows.map(([k,v])=>`<div class="effect-row"><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('');
  }
  function ensureStatusLayout038(){
    const panel=document.querySelector('.hero-stats'); if(!panel) return;
    let layout=panel.querySelector('.status-layout');
    if(!layout){
      layout=document.createElement('div'); layout.className='status-layout';
      const left=document.createElement('div'); left.className='status-left';
      const right=document.createElement('div'); right.className='status-right';
      const grid=panel.querySelector('.stat-grid'); if(grid) left.appendChild(grid);
      let special=$id('statusSpecialEffects') || panel.querySelector('.status-special-effects'); if(special) left.appendChild(special);
      let records=panel.querySelector('.monster-records'); if(records) right.appendChild(records);
      layout.append(left,right); panel.appendChild(layout);
    }
    const left=layout.querySelector('.status-left') || layout.appendChild(Object.assign(document.createElement('div'),{className:'status-left'}));
    const right=layout.querySelector('.status-right') || layout.appendChild(Object.assign(document.createElement('div'),{className:'status-right'}));
    const grid=panel.querySelector(':scope > .stat-grid') || panel.querySelector('.stat-grid'); if(grid && grid.parentElement!==left) left.insertBefore(grid,left.firstChild);
    let special=$id('statusSpecialEffects') || panel.querySelector('.status-special-effects');
    if(!special){ special=document.createElement('div'); special.id='statusSpecialEffects'; special.className='status-special-effects'; special.innerHTML='<h3>特殊効果</h3><div class="effect-scroll"></div>'; }
    if(special.parentElement!==left) left.appendChild(special);
    let records=panel.querySelector('.monster-records'); if(records && records.parentElement!==right) right.appendChild(records);
  }
  function ensureEquipLayout038(){
    const panel=document.querySelector('.equip-panel'); if(!panel) return;
    let layout=panel.querySelector('.equip-layout');
    if(!layout){
      layout=document.createElement('div'); layout.className='equip-layout';
      layout.innerHTML='<div class="equip-left"></div><div class="equip-right"></div>'; panel.appendChild(layout);
    }
    const left=layout.querySelector('.equip-left') || layout.appendChild(Object.assign(document.createElement('div'),{className:'equip-left'}));
    const right=layout.querySelector('.equip-right') || layout.appendChild(Object.assign(document.createElement('div'),{className:'equip-right'}));
    let btn=$id('bestEquipBtnEquip');
    if(!btn){ btn=document.createElement('button'); btn.id='bestEquipBtnEquip'; btn.type='button'; btn.className='wide best-equip-in-equip'; btn.textContent='最強装備'; }
    if(btn.parentElement!==left) left.insertBefore(btn,left.firstChild);
    if(!btn.__mbh038){ btn.__mbh038=true; btn.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); if(typeof bestEquip==='function') bestEquip(); }); return false;}; }
    const list=$id('equipList'); if(list && list.parentElement!==left) left.appendChild(list);
    const up=$id('upgradeBtn'); if(up && up.parentElement!==left) left.appendChild(up);
    let totals=$id('equipEffectTotals') || panel.querySelector('.equip-effect-totals');
    if(!totals){ totals=document.createElement('div'); totals.id='equipEffectTotals'; totals.className='equip-effect-totals'; totals.innerHTML='<h3>装備効果合計</h3><div class="effect-scroll"></div>'; }
    if(totals.parentElement!==right) right.appendChild(totals);
    $$('.equip-effect-totals').forEach(x=>{ if(x!==totals) x.remove(); });
  }
  function renderEffects038(){
    removeEnemyXpLine038(); ensureStatusLayout038(); ensureEquipLayout038();
    safe(()=>{
      const st=(typeof calcStats==='function')?calcStats():{};
      const box=document.querySelector('#statusSpecialEffects .effect-scroll'); if(box) box.innerHTML=rowsHtml038(labelRowsFromStats(st),'特殊効果なし');
    });
    safe(()=>{
      const t=equipmentTotals038(); const rows=[];
      const addFlat=(name,v)=>{ if(v) rows.push([name,(v>0?'+':'')+Math.floor(v).toLocaleString()]); };
      const addPct=(name,v)=>{ if(v) rows.push([name,pct(v)]); };
      addFlat('HP',t.hp); addFlat('攻撃力',t.atk); addFlat('防御力',t.def); addPct('火軽減',t.fireRes); addPct('火吸収',t.fireDamageHeal); addPct('雷ダメージ',t.thunderDmg); addPct('雷属性強化',t.thunderSkillChance); addPct('死線の剣舞率',t.deathDanceChance); addPct('HP吸収',t.lifeSteal); addPct('ガード',t.guard); addPct('会心',t.crit); addPct('防御無視',t.deathDanceDefIgnore); addPct('暗黒出血付与',t.heroDarkBleedChance);
      const box=document.querySelector('#equipEffectTotals .effect-scroll'); if(box) box.innerHTML=rowsHtml038(rows,'装備効果なし');
    });
  }

  function classify038(msg,cls){
    const text=String(msg||'').replace(/<[^>]+>/g,''); const c=String(cls||'');
    if(/ドロップ|獲得|報酬|枠目/.test(text) || /drop|rarity|log-item/.test(c)) return 'drop';
    if(/ダメージ|被回復|回復|吸収|出血|暗黒出血|火傷|GUARD|反射|軽減|会心|CRIT|斬撃|炎斬り|雷撃|攻撃/.test(text) || /damage|danger|good|skilllog|heal|fire|thunder|dark|crit/.test(c)) return 'damage';
    return 'system';
  }
  function renderLog038(reason='append'){
    const box=$id('log'); if(!box || typeof state==='undefined') return;
    const filter=state.logFilter||'all';
    const wasBottom=(box.scrollTop+box.clientHeight>=box.scrollHeight-8);
    const rows=(state.log||[]).map(l=>{ l.type=classify038(l.msg,l.cls); return l; }).filter(l=>filter==='all'||l.type===filter);
    box.innerHTML=rows.map(l=>`<div class="${esc(l.cls||'')}" data-log-type="${esc(l.type)}"><b>[${esc(l.time||'')}]</b> ${l.html?String(l.msg):esc(l.msg)}</div>`).join('');
    const bar=$id('logFilterBar'); if(bar){
      if(!bar.__mbh038){
        bar.__mbh038=true; bar.innerHTML='<button type="button" data-log-filter="all">すべて</button><button type="button" data-log-filter="damage">ダメージ</button><button type="button" data-log-filter="system">システム</button><button type="button" data-log-filter="drop">ドロップ</button>';
        $$('button[data-log-filter]',bar).forEach(b=>{ const on=(e)=>{ e.preventDefault(); e.stopPropagation(); state.logFilter=b.dataset.logFilter||'all'; renderLog038('filter'); return false;}; b.onclick=on; b.ontouchend=on; });
      }
      $$('button[data-log-filter]',bar).forEach(b=>b.classList.toggle('active',(b.dataset.logFilter||'all')===filter));
    }
    if(reason==='filter' || wasBottom) box.scrollTop=box.scrollHeight;
  }
  function installLog038(){
    if(typeof state==='undefined') return;
    log=function(msg,cls='',html=false){
      const time=new Date().toLocaleTimeString('ja-JP',{hour12:false});
      const safeMsg=html?String(msg):esc(msg);
      const type=classify038(safeMsg,cls);
      state.log.unshift({time,msg:safeMsg,cls,html:true,type});
      state.log=state.log.slice(0,220);
      renderLog038('append');
    };
    window.log=log; renderLog038('filter');
  }

  function sellExpValue038(it){ const base=it?.rarity==='legendary'?5:(it?.rarity==='rare'?3:1); const plus=Math.max(0,Math.floor(Number(it?.level)||0)); return base+plus; }
  function installSell038(){
    safe(()=>{ sellExpValue=sellExpValue038; window.sellExpValue=sellExpValue038; });
    safe(()=>{ updateSellButtonState=function(){ const btn=$id('sellSelectedBtn'); if(!btn || typeof state==='undefined') return; const targets=[]; if($id('sellNormalChk')?.checked) targets.push('normal'); if($id('sellRareChk')?.checked) targets.push('rare'); if($id('sellLegendaryChk')?.checked) targets.push('legendary'); const items=(state.inventory||[]).filter(it=>targets.includes(it.rarity)&&!it.unsellable); const xp=items.reduce((s,it)=>s+sellExpValue038(it),0); btn.disabled=targets.length===0; btn.textContent=`経験値化 (${items.length}) +${xp}`; }; window.updateSellButtonState=updateSellButtonState; updateSellButtonState(); });
  }

  function patchRender038(){
    if(!window.__mbh038RenderStats && typeof renderStats==='function'){
      window.__mbh038RenderStats=renderStats;
      renderStats=function(){ const r=window.__mbh038RenderStats.apply(this,arguments); renderEffects038(); return r; };
    }
    if(!window.__mbh038RenderEquip && typeof renderEquip==='function'){
      window.__mbh038RenderEquip=renderEquip;
      renderEquip=function(){ const r=window.__mbh038RenderEquip.apply(this,arguments); renderEffects038(); return r; };
    }
    if(!window.__mbh038RenderBattle && typeof renderBattle==='function'){
      window.__mbh038RenderBattle=renderBattle;
      renderBattle=function(){ const r=window.__mbh038RenderBattle.apply(this,arguments); removeEnemyXpLine038(); return r; };
    }
  }

  function boot038(){
    syncVersion038(); installMenuToggle038(); syncResponsive038(); ensureFooter038(); installTabs038(); setMenuPage038((typeof state!=='undefined'&&state.menuPage)||'stats'); renderEffects038(); installLog038(); installSell038(); patchRender038(); removeEnemyXpLine038();
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot038,{once:true}); else setTimeout(boot038,0);
  window.addEventListener('load',()=>{ boot038(); setTimeout(boot038,80); setTimeout(boot038,400); },{once:true});
  window.addEventListener('resize',()=>{ syncResponsive038(); });
  window.addEventListener('orientationchange',()=>{ syncResponsive038(); });
  setInterval(()=>{ safe(()=>{ syncVersion038(); syncResponsive038(); ensureFooter038(); removeEnemyXpLine038(); renderEffects038(); installMenuToggle038(); installTabs038(); }); },1500);
...
/* ver.0.5.3: final stable layout, no insertBefore loop, null enemy guard, log category hard fix */
(function(){
  'use strict';
  const APP_VERSION = '0.5.3';
  const $ = (id)=>document.getElementById(id);
  ...
  function safe(fn,label){ try{return fn();}catch(e){ console.error('[MBH 0.5.3 '+(label||'')+']', e); } }
  function syncVersion039(){
    ...
    safe(()=>$$('.debug-version').forEach(el=>{ el.textContent='Build: ver.'+APP_VERSION; }));
    safe(()=>{ const tt=document.querySelector('#mbhTraceBox .debug-trace-title'); if(tt) tt.textContent='進行デバッグログ ver.'+APP_VERSION; });
  }
  function setHtmlStable(el, html){ if(el && el.__mbhLastHtml039 !== html){ el.__mbhLastHtml039 = html; el.innerHTML = html; } }
  function effectRowsHtml(rows, empty='効果なし'){
    if(!rows || !rows.length) return '<div class="effect-empty">'+esc(empty)+'</div>';
    return rows.map(r=>'<div class="effect-row"><span>'+esc(r[0])+'</span><b>'+esc(r[1])+'</b></div>').join('');
  }
  function pct(v){ return Math.round((Number(v)||0)*100)+'%'; }
  function rowsFromStats(){
    const rows=[]; if(typeof calcStats !== 'function') return rows;
    const st=calcStats()||{};
    const add=(name,val,mode)=>{ if(val){ rows.push([name, (mode==='plus'?'+':'')+pct(val)]); } };
    add('火軽減', st.fireRes); add('炎被ダメ回復', st.fireDamageHeal); add('火ダメージ', st.fireDmg, 'plus'); add('炎斬り発動率', st.fireSkillChance, 'plus');
    add('雷ダメージ', st.thunderDmg, 'plus'); add('雷撃発動率', st.thunderSkillChance, 'plus');
    add('剣舞発動率', st.deathDanceChance); add('剣舞防御無視', st.deathDanceDefIgnore); add('暗黒出血付与', st.heroDarkBleedChance);
    add('吸収', st.lifeSteal); add('ガード', st.guard); add('会心', st.crit);
    if(st.deathDanceDurationMul && st.deathDanceDurationMul!==1) rows.push(['剣舞時間', Math.round(st.deathDanceDurationMul*100)+'%']);
    if(st.masterRegen) rows.push(['師匠のアミュレット', ((Number(st.masterRegenRate)||0)*100).toFixed(1)+'% / 10秒']);
    if(st.darkShield) rows.push(['闇の盾', '有効']);
    if(st.darkAmulet) rows.push(['闇のアミュレット', '有効']);
    const eq=(typeof state!=='undefined' && state.equip) ? state.equip : {};
    if(Object.values(eq).some(it=>it && /闇の兜/.test(String(it.name||'')))) rows.push(['闇の兜', '剣舞+10% / 出血解除 / 剣舞中無効']);
    if(Object.values(eq).some(it=>it && /闇の籠手/.test(String(it.name||'')))) rows.push(['闇の籠手', '剣舞回数+2']);
    if(Object.values(eq).some(it=>it && /闇の靴/.test(String(it.name||'')))) rows.push(['闇の靴', '回避+25% / 瀕死時+25%']);
    if(Object.values(eq).some(it=>it && /闇の鎧/.test(String(it.name||'')))) rows.push(['闇の鎧', '軽減25% / 自動回復 / 暗黒出血上限20']);
    return rows;
  }
  function equipmentTotalRows(){
    const totals={};
    const eq=(typeof state!=='undefined' && state.equip) ? state.equip : {};
    const add=(k,v)=>{ v=Number(v)||0; if(!v) return; totals[k]=(totals[k]||0)+v; };
    Object.values(eq).filter(Boolean).forEach(it=>{
      add('HP', it.hp); add('攻撃力', it.atk); add('防御力', it.def);
      add('火軽減', it.fireRes); add('火ダメージ', it.fireDmg); add('炎斬り発動率', it.fireSkillChance); add('炎被ダメ回復', it.fireDamageHeal);
      add('雷ダメージ', it.thunderDmg); add('雷撃発動率', it.thunderSkillChance);
      add('剣舞発動率', it.deathDanceChance); add('剣舞防御無視', it.deathDanceDefIgnore); add('暗黒出血付与', it.heroDarkBleedChance);
      add('吸収', it.lifeSteal); add('ガード', it.guard); add('会心', it.crit);
      add('撃破時回復', it.killHeal);
    });
    const percentKeys = new Set(['火軽減','火ダメージ','炎斬り発動率','炎被ダメ回復','雷ダメージ','雷撃発動率','剣舞発動率','剣舞防御無視','暗黒出血付与','吸収','ガード','会心','撃破時回復']);
    return Object.entries(totals).map(([k,v])=>[k, percentKeys.has(k)?pct(v):String(Math.floor(v))]);
  }
  function ensureMenuSkeleton039(){
    const side=document.querySelector('.side-panel'); if(!side) return;
    const tabs=document.querySelector('.mobile-menu-tabs.side-tabs') || document.querySelector('.mobile-menu-tabs');
    let content=$('mbhMenuContent039');
    if(!content){ content=document.createElement('div'); content.id='mbhMenuContent039'; content.className='menu-content-fixed'; }
    let footer=$('mbhMenuFooter039');
    if(!footer){ footer=document.createElement('div'); footer.id='mbhMenuFooter039'; footer.className='legal-links menu-footer-fixed'; footer.setAttribute('aria-label','公開情報'); }
    const oldLegal=document.querySelector('.legal-links:not(#mbhMenuFooter039)');
    if(oldLegal){ footer.innerHTML=oldLegal.innerHTML; oldLegal.remove(); }
    else if(!footer.innerHTML.trim()) footer.innerHTML='<button id="creditBtn" type="button">クレジット</button><button id="termsBtn" type="button">利用規約</button><button id="privacyBtn" type="button">プライバシーポリシー</button>';

    // side-panel直下を固定順へ。既存パネルはcontentへ集約。
    if(tabs && tabs.parentNode!==side) side.insertBefore(tabs, side.firstChild);
    ['.hero-stats','.equip-panel','.inventory-panel','.log-panel'].forEach(sel=>{
      const p=document.querySelector(sel); if(p && p.parentNode!==content) content.appendChild(p);
    });
    if(content.parentNode!==side) side.insertBefore(content, footer.parentNode===side?footer:null);
    if(footer.parentNode!==side) side.appendChild(footer);

    // 旧menu-left/rightは空なら削除、残っていてもレイアウト対象外にする
    $$('.menu-col').forEach(c=>{ if(!c.querySelector('.panel')) c.remove(); });
    bindLegal039();
  }
  function bindLegal039(){
    const bind=(id,type)=>{ const b=$(id); if(b && b.__mbhLegal039!=='1'){ b.__mbhLegal039='1'; b.onclick=(e)=>{ e.preventDefault(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal(type); }); }; } };
    bind('creditBtn','credit'); bind('termsBtn','terms'); bind('privacyBtn','privacy');
  }
  function ensureStatusLayout039(){
    const panel=document.querySelector('.hero-stats'); if(!panel) return;
    let layout=panel.querySelector('.status-layout');
    if(!layout){ layout=document.createElement('div'); layout.className='status-layout'; }
    let left=layout.querySelector('.status-left'); if(!left){ left=document.createElement('div'); left.className='status-left'; }
    let right=layout.querySelector('.status-right'); if(!right){ right=document.createElement('div'); right.className='status-right'; }
    let h2=panel.querySelector(':scope > h2') || left.querySelector('h2'); if(!h2){ h2=document.createElement('h2'); h2.textContent='ステータス'; }
    let grid=$('statLv')?.closest('.stat-grid') || panel.querySelector('.stat-grid');
    if(h2.parentNode!==left) left.insertBefore(h2,left.firstChild);
    if(grid && grid.parentNode!==left) left.appendChild(grid);
    removeEnemyLevelXp039();
    let fx=$('statusSpecialEffects'); if(!fx){ fx=document.createElement('div'); fx.id='statusSpecialEffects'; fx.className='status-special-effects'; fx.innerHTML='<h3>特殊効果</h3><div class="effect-scroll"></div>'; }
    if(fx.parentNode!==left) left.appendChild(fx);
    let rec=panel.querySelector('.monster-records'); if(!rec){ rec=document.createElement('div'); rec.className='monster-records'; rec.innerHTML='<h3>モンスター討伐記録</h3><div id="monsterRecords" class="monster-record-list"></div>'; }
    if(rec.parentNode!==right) right.appendChild(rec);
    if(left.parentNode!==layout) layout.appendChild(left);
    if(right.parentNode!==layout) layout.appendChild(right);
    if(layout.parentNode!==panel){ panel.innerHTML=''; panel.appendChild(layout); }
  }
  function ensureEquipLayout039(){
    const panel=document.querySelector('.equip-panel'); if(!panel) return;
    let layout=panel.querySelector('.equip-layout'); if(!layout){ layout=document.createElement('div'); layout.className='equip-layout'; }
    let left=layout.querySelector('.equip-left'); if(!left){ left=document.createElement('div'); left.className='equip-left'; }
    let right=layout.querySelector('.equip-right'); if(!right){ right=document.createElement('div'); right.className='equip-right'; }
    let h2=panel.querySelector(':scope > h2') || left.querySelector('h2'); if(!h2){ h2=document.createElement('h2'); h2.textContent='装備'; } h2.textContent='装備';
    let best=$('bestEquipBtn');
    if(!best){ best=document.createElement('button'); best.id='bestEquipBtn'; best.type='button'; best.textContent='最強装備'; }
    best.classList.add('wide');
    if(best.__mbhBest039!=='1'){ best.__mbhBest039='1'; best.onclick=(e)=>{ e.preventDefault(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof bestEquip==='function') bestEquip(); }); }; }
    let list=$('equipList'); if(!list){ list=document.createElement('div'); list.id='equipList'; list.className='equip-list'; }
    const up=$('upgradeBtn'); if(up){ up.classList.add('hidden'); up.style.display='none'; up.disabled=true; }
    if(h2.parentNode!==left) left.appendChild(h2);
    if(best.parentNode!==left) left.appendChild(best);
    if(list.parentNode!==left) left.appendChild(list);
    let totals=$('equipEffectTotals'); if(!totals){ totals=document.createElement('div'); totals.id='equipEffectTotals'; totals.className='equip-effect-totals'; totals.innerHTML='<h3>装備効果合計</h3><div class="effect-scroll"></div>'; }
    if(totals.parentNode!==right) right.appendChild(totals);
    if(left.parentNode!==layout) layout.appendChild(left);
    if(right.parentNode!==layout) layout.appendChild(right);
    if(layout.parentNode!==panel){ panel.innerHTML=''; panel.appendChild(layout); }
  }
  function removeEnemyLevelXp039(){
    safe(()=>{
      ['statEnemyLevelXp','statEnemyLevelXpLabel'].forEach(id=>{ const el=$(id); if(el) el.remove(); });
      $$('.stat-grid span').forEach(sp=>{ if(/敵Lv経験値|敵Lv経験|敵レベル経験値/.test((sp.textContent||''))){ const n=sp.nextElementSibling; sp.remove(); if(n && n.tagName==='B') n.remove(); } });
    });
  }
  function renderStableExtraPanels039(){
    ensureMenuSkeleton039(); ensureStatusLayout039(); ensureEquipLayout039(); removeEnemyLevelXp039();
    const sbox=document.querySelector('#statusSpecialEffects .effect-scroll'); setHtmlStable(sbox, effectRowsHtml(rowsFromStats(),'特殊効果なし'));
    const ebox=document.querySelector('#equipEffectTotals .effect-scroll'); setHtmlStable(ebox, effectRowsHtml(equipmentTotalRows(),'装備効果なし'));
  }
  function classify039(entry){
    const old=String(entry?.type||entry?.cat||'').toLowerCase();
    const cls=String(entry?.cls||'').toLowerCase();
    const msg=String(entry?.msg||'').replace(/<[^>]*>/g,'');
    if(/ドロップ|報酬|獲得|倉庫に追加|拾った|入手|枠目=/.test(msg) || cls.includes('drop') || old==='drop') return 'drop';
    if(/ダメージ|被回復|回復|吸収|出血|暗黒出血|火傷|暗黒剣舞|暗黒剣技|剣舞|炎斬り|雷撃|連続攻撃|GUARD|無効|反射|ブレス|攻撃/.test(msg) || cls.includes('damage') || cls.includes('skilllog')) return 'damage';
    if(/経験値|LEVEL UP|レベル|敗北|復活|ゲーム開始|デバッグ|逃走|離脱|出現|召喚/.test(msg) || cls.includes('system')) return 'system';
    return old || 'system';
  }
  function renderLog039(reason='append'){
    const el=$('log'); if(!el || typeof state==='undefined') return;
    const filter=state.logFilter||'all';
    const beforeTop=el.scrollTop, beforeHeight=el.scrollHeight;
    const rows=(Array.isArray(state.log)?state.log:[]).filter(l=>filter==='all'||classify039(l)===filter);
    const html=rows.map(l=>'<div class="'+esc(l.cls||'')+'" data-log-type="'+classify039(l)+'">['+esc(l.time||'')+'] '+(l.html?String(l.msg||''):esc(l.msg||''))+'</div>').join('');
    setHtmlStable(el, html);
    $$('#logFilterBar button').forEach(b=>b.classList.toggle('active',(b.dataset.logFilter||'all')===filter));
    requestAnimationFrame(()=>{ if(reason==='filter') el.scrollTop=0; else el.scrollTop=Math.max(0,beforeTop+(el.scrollHeight-beforeHeight)); });
  }
  function patchLog039(){
    if(typeof state==='undefined') return;
    state.logFilter=state.logFilter||'all';
    const oldLog=(typeof log==='function')?log:null;
    log=function(msg, cls='', html=false){
      const time=new Date().toLocaleTimeString('ja-JP',{hour12:false});
      const safeMsg=html?String(msg):esc(msg);
      const entry={time,msg:safeMsg,cls,html:true}; entry.type=classify039(entry);
      state.log.unshift(entry); state.log=state.log.slice(0,180); renderLog039('append');
    };
    const bar=$('logFilterBar'); if(bar){
      if(!bar.__mbh039){
        bar.__mbh039='1';
        bar.innerHTML='<button type="button" data-log-filter="all">すべて</button><button type="button" data-log-filter="damage">ダメージ</button><button type="button" data-log-filter="system">システム</button><button type="button" data-log-filter="drop">ドロップ</button>';
      }
      $$('#logFilterBar button').forEach(b=>{ b.onclick=(e)=>{ e.preventDefault(); state.logFilter=b.dataset.logFilter||'all'; renderLog039('filter'); }; });
    }
    renderLog039('filter');
  }
  function patchRender039(){
    const baseStats=window.__mbhRenderStats9957 || window.__mbhBaseRenderStats039 || (typeof renderStats==='function'?renderStats:null);
    if(baseStats && !window.__mbhBaseRenderStats039) window.__mbhBaseRenderStats039=baseStats;
    if(window.__mbhBaseRenderStats039){ renderStats=function(){ const r=safe(()=>window.__mbhBaseRenderStats039.apply(this,arguments),'baseStats'); safe(renderStableExtraPanels039,'stableStats'); return r; }; }
    const baseEquip=window.__mbhRenderEquip9957 || window.__mbhBaseRenderEquip039 || (typeof renderEquip==='function'?renderEquip:null);
    if(baseEquip && !window.__mbhBaseRenderEquip039) window.__mbhBaseRenderEquip039=baseEquip;
    if(window.__mbhBaseRenderEquip039){ renderEquip=function(){ const r=safe(()=>window.__mbhBaseRenderEquip039.apply(this,arguments),'baseEquip'); safe(renderStableExtraPanels039,'stableEquip'); return r; }; }
  }
  function patchEnemyDefeat039(){
    if(typeof markEnemyDefeated==='function' && !window.__mbhMarkEnemyDefeated039){
      window.__mbhMarkEnemyDefeated039=markEnemyDefeated;
      markEnemyDefeated=function(e){ if(!e || !e.id) return; return window.__mbhMarkEnemyDefeated039.apply(this,arguments); };
    }
    if(typeof enemyDefeated==='function' && !window.__mbhEnemyDefeated039){
      window.__mbhEnemyDefeated039=enemyDefeated;
      let running=false;
      enemyDefeated=function(){
        if(running) return;
        if(typeof state==='undefined' || !state.enemy || !state.enemy.id){ if(typeof spawnEnemy==='function') setTimeout(()=>safe(()=>spawnEnemy(false),'respawn'),120); return; }
        running=true; try{ return window.__mbhEnemyDefeated039.apply(this,arguments); } finally { setTimeout(()=>{ running=false; }, 250); }
      };
    }
  }
  function patchMenu039(){
    if(typeof state!=='undefined') state.uiOpen=false;
    const side=document.querySelector('.side-panel'); if(side) side.classList.remove('open');
    const btn=$('equipToggleBtn');
    if(btn && btn.__mbhMenu039!=='1'){
      btn.__mbhMenu039='1'; btn.textContent='メニュー';
      btn.onclick=(e)=>{ e.preventDefault(); if(typeof state==='undefined') return; const compact=matchMedia('(max-width:1279px), (max-height:700px)').matches; if(!compact) return; state.uiOpen=!state.uiOpen; const s=document.querySelector('.side-panel'); if(s) s.classList.toggle('open',!!state.uiOpen); btn.textContent='メニュー'; safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); };
    }
  }
  function installCss039(){
    if($('mbh-039-final-css')) return;
    const st=document.createElement('style'); st.id='mbh-039-final-css';
    st.textContent=`
      ...
      .menu-content-fixed{display:block!important;min-width:0!important;min-height:0!important;overflow:hidden!important;}
      .menu-content-fixed>.panel{width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;overflow:hidden!important;margin:0!important;}
      .side-panel .panel{width:100%!important;max-width:none!important;justify-self:stretch!important;align-self:stretch!important;}
      .hero-stats.active-page,.equip-panel.active-page,.inventory-panel.active-page,.log-panel.active-page{display:block!important;}
      .inventory-panel.active-page{display:flex!important;flex-direction:column!important;}
      .log-panel.active-page{display:flex!important;flex-direction:column!important;}
      .side-panel .panel:not(.active-page){display:none!important;}
      @media(min-width:1280px) and (min-height:701px){.side-panel .panel{display:block!important}.menu-content-fixed{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important;grid-template-rows:minmax(0,1fr) var(--mbh-log-h,240px)!important;gap:10px!important}.hero-stats{grid-column:1!important;grid-row:1!important}.equip-panel{grid-column:1!important;grid-row:2!important}.inventory-panel{grid-column:2!important;grid-row:1!important}.log-panel{grid-column:2!important;grid-row:2!important}#equipToggleBtn{display:none!important}}
      .status-layout,.equip-layout{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important;gap:12px!important;width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;align-items:stretch!important;}
      .status-left,.status-right,.equip-left,.equip-right{min-width:0!important;min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      .status-left .stat-grid{flex:0 0 auto!important}.status-special-effects,.equip-effect-totals{flex:1 1 auto!important;min-height:0!important;overflow:hidden!important;margin-top:10px!important;}
      .effect-scroll{height:100%!important;max-height:none!important;overflow-y:auto!important;overflow-x:hidden!important;}
      .monster-records{flex:1 1 auto!important;min-height:0!important;overflow:hidden!important;margin-top:0!important}.monster-record-list{height:100%!important;max-height:none!important;overflow-y:auto!important;}
      .equip-list{flex:1 1 auto!important;min-height:0!important;overflow-y:auto!important;grid-template-columns:1fr 1fr!important;}
      #bestEquipBtn{display:block!important;margin:0 0 8px!important}.equip-effect-totals{margin-top:0!important;}
      .inventory-panel .inventory{width:100%!important;flex:1 1 auto!important;min-height:0!important;max-height:none!important;overflow-y:auto!important;}
      .log-panel #log{width:100%!important;flex:1 1 auto!important;min-height:0!important;height:auto!important;overflow-y:auto!important;}
      .menu-footer-fixed{flex:0 0 auto!important;display:flex!important;gap:8px!important;justify-content:center!important;align-items:center!important;padding:8px 0 0!important;border-top:1px solid #513917!important;background:linear-gradient(rgba(8,6,4,.96),rgba(5,5,5,.98))!important;z-index:5!important;position:relative!important;}
      .menu-footer-fixed button{font-size:12px!important;padding:6px 8px!important;}
      #statEnemyLevelXp,#statEnemyLevelXpLabel{display:none!important;}
      body.menu-open .flee-btn, body.menu-open .expbar{display:none!important;}
      @media(max-width:760px){.status-layout,.equip-layout{grid-template-columns:1fr!important}.menu-footer-fixed{flex-wrap:wrap!important}.equip-list{grid-template-columns:1fr!important}}
    `;
    document.head.appendChild(st);
  }
  function boot039(){
    syncVersion039(); installCss039(); patchMenu039(); patchEnemyDefeat039(); patchRender039(); patchLog039(); renderStableExtraPanels039();
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot039, {once:true}); else setTimeout(boot039,0);
  window.addEventListener('load', boot039, {once:true});
  setInterval(()=>safe(()=>{ syncVersion039(); removeEnemyLevelXp039(); },'tick'),1500);
})();


/* MBH ver.0.5.3: 軽量化・ログ撤去・メニューDOM固定 */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(id)=>document.getElementById(id);
  const $$=(sel,root=document)=>Array.from(root.querySelectorAll(sel));
  const safe=(fn)=>{ try{return fn&&fn();}catch(e){ console.warn('[MBH 0.5.3]', e); } };
  ...
  function syncVersion041(){
    document.documentElement.dataset.buildVersion=BUILD;
    const b=document.querySelector('.build-version'); if(b) b.textContent='ver.'+BUILD;
    const d=document.querySelector('.debug-version'); if(d) d.textContent='Build: ver.'+BUILD+' debug';
  }

  function installCss041(){
    ...
    st.textContent=`
      ...
      .menu-content-fixed{display:block!important;width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;overflow:hidden!important;}
      .menu-content-fixed>.panel{width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;overflow:hidden!important;margin:0!important;justify-self:stretch!important;align-self:stretch!important;}
      .side-panel .panel:not(.active-page){display:none!important;}
      .side-panel .panel.active-page{display:block!important;}
      .inventory-panel.active-page{display:flex!important;flex-direction:column!important;}
      @media(min-width:1280px) and (min-height:701px){
        #equipToggleBtn{display:none!important;}
        .side-panel{position:static!important;display:grid!important;width:auto!important;background:none!important;border:0!important;padding:0!important;box-shadow:none!important;}
        .side-panel .panel{display:block!important;}
        .menu-content-fixed{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important;grid-template-rows:minmax(0,1fr) minmax(0,1fr)!important;gap:10px!important;}
        .hero-stats{grid-column:1!important;grid-row:1!important;}
        .equip-panel{grid-column:1!important;grid-row:2!important;}
        .inventory-panel{grid-column:2!important;grid-row:1 / 3!important;display:flex!important;flex-direction:column!important;}
        .log-panel{display:none!important;}
      }
      .status-layout,.equip-layout{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important;gap:12px!important;width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;align-items:stretch!important;}
      .status-left,.status-right,.equip-left,.equip-right{min-width:0!important;min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      .status-left .stat-grid{flex:0 0 auto!important;}
      .status-special-effects,.equip-effect-totals{flex:1 1 auto!important;min-height:0!important;overflow:hidden!important;margin-top:10px!important;}
      .effect-scroll{height:100%!important;max-height:none!important;overflow-y:auto!important;overflow-x:hidden!important;}
      .monster-records{flex:1 1 auto!important;min-height:0!important;overflow:hidden!important;margin-top:0!important;}
      .monster-record-list{height:100%!important;max-height:none!important;overflow-y:auto!important;}
      .equip-list{flex:1 1 auto!important;min-height:0!important;overflow-y:auto!important;grid-template-columns:1fr 1fr!important;}
      #bestEquipBtn{display:block!important;margin:0 0 8px!important;}
      .equip-effect-totals{margin-top:0!important;}
      .inventory-panel .inventory{width:100%!important;flex:1 1 auto!important;min-height:0!important;max-height:none!important;overflow-y:auto!important;}
      .menu-footer-fixed{flex:0 0 auto!important;display:flex!important;gap:8px!important;justify-content:center!important;align-items:center!important;padding:8px 0 0!important;border-top:1px solid #513917!important;background:linear-gradient(rgba(8,6,4,.96),rgba(5,5,5,.98))!important;z-index:5!important;position:relative!important;}
      .menu-footer-fixed button{font-size:12px!important;padding:6px 8px!important;}
      #statEnemyLevelXp,#statEnemyLevelXpLabel{display:none!important;}
  ...
  function removeLogUi041(){
    const logBtn=document.querySelector('[data-menu-page="log"]'); if(logBtn) logBtn.remove();
    const lp=document.querySelector('.log-panel'); if(lp) lp.remove();
    const bar=$('logFilterBar'); if(bar) bar.remove();
    const logEl=$('log'); if(logEl) logEl.remove();
    if(typeof state!=='undefined'){
      state.log=[]; state.logFilter='all';
    }
  }

  function installNoopLog041(){
    // ログは完全撤去。内部の呼び出し互換だけ残してDOMを触らない。
    window.__mbh041DroppedLogs=0;
    log=function(){ window.__mbh041DroppedLogs++; return null; };
    // 過去パッチのrenderLog系も無害化
    ['renderLogFiltered','renderLogFiltered9922','renderLogPreserve','renderLog031','renderLog032','renderLog033','renderLog035','renderLog038','renderLog039'].forEach(n=>{ try{ window[n]=function(){}; }catch(e){} });
  }

  function ensureMenuSkeleton041(){
    const side=document.querySelector('.side-panel'); if(!side) return;
    const tabs=document.querySelector('.mobile-menu-tabs.side-tabs') || document.querySelector('.mobile-menu-tabs');
    let content=$('mbhMenuContent041') || $('mbhMenuContent039');
    if(!content){ content=document.createElement('div'); content.id='mbhMenuContent041'; content.className='menu-content-fixed'; }
    content.id='mbhMenuContent041'; content.className='menu-content-fixed';
    let footer=$('mbhMenuFooter041') || $('mbhMenuFooter039');
    if(!footer){ footer=document.createElement('div'); footer.id='mbhMenuFooter041'; footer.className='legal-links menu-footer-fixed'; footer.setAttribute('aria-label','公開情報'); }
    footer.id='mbhMenuFooter041'; footer.className='legal-links menu-footer-fixed';
    $$('.legal-links').forEach(x=>{ if(x!==footer){ if(!footer.innerHTML.trim()) footer.innerHTML=x.innerHTML; x.remove(); } });
    if(!footer.innerHTML.trim()) footer.innerHTML='<button id="creditBtn" type="button">クレジット</button><button id="termsBtn" type="button">利用規約</button><button id="privacyBtn" type="button">プライバシーポリシー</button>';
    if(tabs && tabs.parentNode!==side) side.insertBefore(tabs, side.firstChild);
    ['.hero-stats','.equip-panel','.inventory-panel'].forEach(sel=>{ const p=document.querySelector(sel); if(p && p.parentNode!==content) content.appendChild(p); });
    $$('.log-panel').forEach(x=>x.remove());
    if(content.parentNode!==side) side.insertBefore(content, footer.parentNode===side?footer:null);
    if(footer.parentNode!==side) side.appendChild(footer);
    $$('.menu-col').forEach(c=>{ if(!c.querySelector('.panel')) c.remove(); });
    bindLegal041();
  ...
  function bindLegal041(){
    const bind=(id,type)=>{ const b=$(id); if(b){ b.onclick=(e)=>{ e.preventDefault(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal(type); }); }; } };
    bind('creditBtn','credit'); bind('termsBtn','terms'); bind('privacyBtn','privacy');
  }

  function removeEnemyLevelXp041(){
    ['statEnemyLevelXp','statEnemyLevelXpLabel'].forEach(id=>{ const el=$(id); if(el) el.remove(); });
    $$('.stat-grid span').forEach(sp=>{ if(/敵Lv経験値|敵Lv経験|敵レベル経験値/.test(sp.textContent||'')){ const n=sp.nextElementSibling; sp.remove(); if(n && n.tagName==='B') n.remove(); } });
  }

  function ensureStatusLayout041(){
    const panel=document.querySelector('.hero-stats'); if(!panel) return;
    let layout=panel.querySelector('.status-layout') || document.createElement('div'); layout.className='status-layout';
    let left=layout.querySelector('.status-left') || document.createElement('div'); left.className='status-left';
    let right=layout.querySelector('.status-right') || document.createElement('div'); right.className='status-right';
    let h2=panel.querySelector(':scope > h2') || left.querySelector('h2') || document.createElement('h2'); h2.textContent='ステータス';
    const grid=$('statLv')?.closest('.stat-grid') || panel.querySelector('.stat-grid');
    if(h2.parentNode!==left) left.insertBefore(h2,left.firstChild);
    if(grid && grid.parentNode!==left) left.appendChild(grid);
    removeEnemyLevelXp041();
    let fx=$('statusSpecialEffects') || document.createElement('div'); fx.id='statusSpecialEffects'; fx.className='status-special-effects';
    if(!fx.querySelector('h3')) fx.innerHTML='<h3>特殊効果</h3><div class="effect-scroll"></div>';
    if(fx.parentNode!==left) left.appendChild(fx);
    let rec=panel.querySelector('.monster-records') || document.createElement('div'); rec.className='monster-records';
    if(!rec.querySelector('#monsterRecords')) rec.innerHTML='<h3>モンスター討伐記録</h3><div id="monsterRecords" class="monster-record-list"></div>';
    if(rec.parentNode!==right) right.appendChild(rec);
    if(left.parentNode!==layout) layout.appendChild(left);
    if(right.parentNode!==layout) layout.appendChild(right);
    if(layout.parentNode!==panel){ panel.innerHTML=''; panel.appendChild(layout); }
  ...
  function ensureEquipLayout041(){
    const panel=document.querySelector('.equip-panel'); if(!panel) return;
    let layout=panel.querySelector('.equip-layout') || document.createElement('div'); layout.className='equip-layout';
    let left=layout.querySelector('.equip-left') || document.createElement('div'); left.className='equip-left';
    let right=layout.querySelector('.equip-right') || document.createElement('div'); right.className='equip-right';
    let h2=panel.querySelector(':scope > h2') || left.querySelector('h2') || document.createElement('h2'); h2.textContent='装備';
    let best=$('bestEquipBtn'); if(!best){ best=document.createElement('button'); best.id='bestEquipBtn'; best.type='button'; best.textContent='最強装備'; }
    best.classList.add('wide'); best.onclick=(e)=>{ e.preventDefault(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof bestEquip==='function') bestEquip(); }); };
    let list=$('equipList') || document.createElement('div'); list.id='equipList'; list.className='equip-list';
    const up=$('upgradeBtn'); if(up){ up.classList.add('hidden'); up.style.display='none'; up.disabled=true; }
    if(h2.parentNode!==left) left.appendChild(h2);
    if(best.parentNode!==left) left.appendChild(best);
    if(list.parentNode!==left) left.appendChild(list);
    let totals=$('equipEffectTotals') || document.createElement('div'); totals.id='equipEffectTotals'; totals.className='equip-effect-totals';
    if(!totals.querySelector('h3')) totals.innerHTML='<h3>装備効果合計</h3><div class="effect-scroll"></div>';
    if(totals.parentNode!==right) right.appendChild(totals);
    if(left.parentNode!==layout) layout.appendChild(left);
    if(right.parentNode!==layout) layout.appendChild(right);
    if(layout.parentNode!==panel){ panel.innerHTML=''; panel.appendChild(layout); }
  ...
  function effectRowsHtml(rows,empty){
    if(!rows || !rows.length) return '<div class="effect-empty">'+esc(empty)+'</div>';
    return rows.map(([k,v])=>'<div class="effect-row"><span>'+esc(k)+'</span><b>'+esc(v)+'</b></div>').join('');
  }

  function equipmentItems(){ return Object.values((typeof state!=='undefined' && state.equip) ? state.equip : {}).filter(Boolean); }
  function addRow(rows,k,v,percent=false){ v=Number(v)||0; if(!v) return; rows.push([k,percent?pct(v):(v>0?'+'+num(v):num(v))]); }
  function stableEffectRows041(){
    // 装備そのものだけを見る。一時バフ・攻撃中・被弾中の値は見ない。
    const rows=[]; const items=equipmentItems();
    let totals={}; const add=(k,v)=>{ v=Number(v)||0; if(v) totals[k]=(totals[k]||0)+v; };
    for(const it of items){
      add('火軽減',it.fireRes); add('火ダメージ',it.fireDmg); add('炎斬り発動率',it.fireSkillChance); add('炎被ダメ回復',it.fireDamageHeal);
      add('雷ダメージ',it.thunderDmg); add('雷撃発動率',it.thunderSkillChance); add('剣舞発動率',it.deathDanceChance); add('剣舞防御無視',it.deathDanceDefIgnore);
      add('暗黒出血付与',it.heroDarkBleedChance); add('吸収',it.lifeSteal); add('ガード',it.guard); add('会心',it.crit);
    }
    const percentKeys=['火軽減','火ダメージ','炎斬り発動率','炎被ダメ回復','雷ダメージ','雷撃発動率','剣舞発動率','剣舞防御無視','暗黒出血付与','吸収','ガード','会心'];
    for(const k of percentKeys){ if(totals[k]) rows.push([k,(k==='火軽減'?'':'+')+pct(totals[k])]); }
    if(items.some(it=>/師匠のアミュレット/.test(String(it.name||'')))) rows.push(['師匠のアミュレット','10秒ごとにHP回復 / 撃破時回復']);
    if(items.some(it=>/闇の盾/.test(String(it.name||'')))) rows.push(['闇の盾','被ダメ軽減+回復']);
    if(items.some(it=>/闇のアミュレット/.test(String(it.name||'')))) rows.push(['闇のアミュレット','剣舞時間2倍']);
    if(items.some(it=>/闇の兜/.test(String(it.name||'')))) rows.push(['闇の兜','剣舞+10% / 出血解除']);
    if(items.some(it=>/闇の籠手/.test(String(it.name||'')))) rows.push(['闇の籠手','剣舞回数+2']);
    if(items.some(it=>/闇の靴/.test(String(it.name||'')))) rows.push(['闇の靴','回避+25%']);
    if(items.some(it=>/闇の鎧/.test(String(it.name||'')))) rows.push(['闇の鎧','軽減25% / 自動回復']);
    return rows;
  }

  function equipTotalRows041(){
    const totals={}; const add=(k,v)=>{ v=Number(v)||0; if(v) totals[k]=(totals[k]||0)+v; };
    ...
    const percent=new Set(['火軽減','火ダメージ','炎斬り発動率','炎被ダメ回復','雷ダメージ','雷撃発動率','剣舞発動率','剣舞防御無視','暗黒出血付与','吸収','ガード','会心']);
    return Object.entries(totals).map(([k,v])=>[k,percent.has(k)?pct(v):num(v)]);
  }

  let lastEquipHash041='';
  function equipHash041(){ try{return JSON.stringify((typeof state!=='undefined'?state.equip:{}),Object.keys((typeof state!=='undefined'?state.equip:{})).sort());}catch(e){return String(Date.now());} }
  function renderStableExtra041(force=false){
    ensureMenuSkeleton041(); ensureStatusLayout041(); ensureEquipLayout041(); removeLogUi041(); removeEnemyLevelXp041();
    const h=equipHash041(); if(!force && h===lastEquipHash041) return; lastEquipHash041=h;
    setStable(document.querySelector('#statusSpecialEffects .effect-scroll'), effectRowsHtml(stableEffectRows041(),'特殊効果なし'));
    setStable(document.querySelector('#equipEffectTotals .effect-scroll'), effectRowsHtml(equipTotalRows041(),'装備効果なし'));
  }

  function patchRender041(){
    // 0.3.xで積み重なったrenderStats/renderEquipラッパーを避け、0.1.3相当の基礎表示に戻す。
    renderStats=function(){
      safe(()=>{ const st=calcStats(); const need=effectiveXpNext();
        if(els.statLv) els.statLv.textContent=state.level;
        if(els.statXp) els.statXp.textContent=state.xp+' / '+need;
        if(els.statXpNext) els.statXpNext.textContent=need;
        if(els.statXpGain) els.statXpGain.textContent=formatExpDelta(state.lastXpGain);
        if(els.statAtk) els.statAtk.textContent=Math.floor(st.atk);
        if(els.statDef) els.statDef.textContent=Math.floor(st.def);
        if(els.statFireRes) els.statFireRes.textContent=Math.round(st.fireRes*100)+'%';
        if(typeof renderMonsterRecords==='function') renderMonsterRecords();
      });
      renderStableExtra041(false);
    };
    const oldEquip = typeof renderEquip==='function' ? renderEquip : null;
    renderEquip=function(){ if(oldEquip) safe(()=>oldEquip()); renderStableExtra041(true); };
    renderAll=function(){ safe(()=>renderBattle()); safe(()=>renderStats()); safe(()=>renderEquip()); safe(()=>renderInventory()); };
  }

  function patchEnemyDefeat041(){
    if(typeof markEnemyDefeated==='function'){
      const old=markEnemyDefeated;
      markEnemyDefeated=function(e){ if(!e || !e.id) return; return old.apply(this,arguments); };
    }
    if(typeof enemyDefeated==='function'){
      const old=enemyDefeated; let running=false;
      enemyDefeated=function(){
        if(running) return;
        if(typeof state==='undefined' || !state.enemy || !state.enemy.id){ if(typeof spawnEnemy==='function') setTimeout(()=>safe(()=>spawnEnemy(false)),120); return; }
        running=true; try{return old.apply(this,arguments);} finally{ setTimeout(()=>{running=false;},250); }
      };
    }
  }

  function patchMenu041(){
    if(typeof state!=='undefined') state.uiOpen=false;
    const side=document.querySelector('.side-panel'); if(side) side.classList.remove('open');
    document.body.classList.remove('menu-open');
    const btn=$('equipToggleBtn'); if(btn){ btn.textContent='メニュー'; btn.onclick=(e)=>{ e.preventDefault(); const compact=matchMedia('(max-width:1279px), (max-height:700px)').matches; if(!compact) return; state.uiOpen=!state.uiOpen; const s=document.querySelector('.side-panel'); if(s) s.classList.toggle('open',!!state.uiOpen); document.body.classList.toggle('menu-open',!!state.uiOpen); btn.textContent='メニュー'; safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); }; }
  }

  function patchSellExp041(){
    sellExpValue=function(it){ const base=it?.rarity==='legendary'?5:(it?.rarity==='rare'?3:1); const plus=Math.max(0,Math.floor(Number(it?.level)||0)); return base+plus; };
    if(typeof updateSellButtonState==='function') safe(()=>updateSellButtonState());
  }

  function boot041(){
    syncVersion041(); installCss041(); installNoopLog041(); removeLogUi041(); patchMenu041(); patchEnemyDefeat041(); patchSellExp041(); patchRender041(); renderStableExtra041(true);
    safe(()=>{ if(typeof renderAll==='function') renderAll(); });
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot041, {once:true}); else setTimeout(boot041,0);
  window.addEventListener('load', boot041, {once:true});
  setInterval(()=>safe(()=>{ syncVersion041(); removeLogUi041(); removeEnemyLevelXp041(); }),3000);
})();

/* MBH ver.0.5.3: menu layout rule finalizer */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(id)=>document.getElementById(id);
  const $$=(sel,root=document)=>Array.from(root.querySelectorAll(sel));
  const safe=(fn)=>{ try{return fn&&fn();}catch(e){ console.warn('[MBH 0.5.3 layout]', e); } };
  ...
  function installCss041Final(){
    ...
    st.textContent=`
      html[data-build-version="0.5.3"] .log-panel,
      html[data-build-version="0.5.3"] #log,
      html[data-build-version="0.5.3"] #logFilterBar,
      html[data-build-version="0.5.3"] [data-menu-page="log"]{display:none!important;}

      html[data-build-version="0.5.3"] .side-panel{
        box-sizing:border-box!important;
      ...
      html[data-build-version="0.5.3"] .side-panel *{box-sizing:border-box!important;}
      html[data-build-version="0.5.3"] .side-panel>.mobile-menu-tabs{
        display:grid!important;
      ...
      html[data-build-version="0.5.3"] .side-panel>.mobile-menu-tabs button{min-width:0!important;width:100%!important;}

      html[data-build-version="0.5.3"] .menu-col,
      html[data-build-version="0.5.3"] .menu-left,
      html[data-build-version="0.5.3"] .menu-right,
      html[data-build-version="0.5.3"] .menu-main{display:contents!important;width:auto!important;height:auto!important;max-width:none!important;max-height:none!important;}

      html[data-build-version="0.5.3"] #mbhMenuContent041,
      html[data-build-version="0.5.3"] #mbhMenuContent040,
      html[data-build-version="0.5.3"] #mbhMenuContent039,
      html[data-build-version="0.5.3"] .menu-content-fixed{
        width:100%!important;
      ...
      html[data-build-version="0.5.3"] .side-panel .panel{
        width:100%!important;
        height:100%!important;
        min-width:0!important;
        min-height:0!important;
      ...
      html[data-build-version="0.5.3"] .side-panel .panel:not(.active-page){display:none!important;}
      html[data-build-version="0.5.3"] .side-panel .panel.active-page{display:flex!important;flex-direction:column!important;}

      html[data-build-version="0.5.3"] .status-layout,
      html[data-build-version="0.5.3"] .equip-layout{
      ...
      html[data-build-version="0.5.3"] .status-left,
      html[data-build-version="0.5.3"] .status-right,
      html[data-build-version="0.5.3"] .equip-left,
      html[data-build-version="0.5.3"] .equip-right{
        min-width:0!important;
        min-height:0!important;
        max-width:none!important;
        overflow:hidden!important;
      ...
      html[data-build-version="0.5.3"] .hero-stats>h2,
      html[data-build-version="0.5.3"] .equip-panel>h2{flex:0 0 auto!important;}
      html[data-build-version="0.5.3"] .status-left .stat-grid{flex:0 0 auto!important;}
      html[data-build-version="0.5.3"] .status-special-effects,
      html[data-build-version="0.5.3"] .monster-records,
      html[data-build-version="0.5.3"] .equip-effect-totals{
        flex:1 1 auto!important;
        min-height:0!important;
        max-height:none!important;
        overflow:hidden!important;
        border:1px solid rgba(255,215,107,.35)!important;
        border-radius:8px!important;
        padding:8px!important;
        background:rgba(0,0,0,.18)!important;
      }
      html[data-build-version="0.5.3"] .status-special-effects{margin-top:10px!important;}
      html[data-build-version="0.5.3"] .monster-records,
      html[data-build-version="0.5.3"] .equip-effect-totals{margin-top:0!important;}
      html[data-build-version="0.5.3"] .effect-scroll,
      html[data-build-version="0.5.3"] .monster-record-list{
        width:100%!important;
        flex:1 1 auto!important;
        min-height:0!important;
        max-height:none!important;
      ...
      html[data-build-version="0.5.3"] .effect-scroll::-webkit-scrollbar,
      html[data-build-version="0.5.3"] .monster-record-list::-webkit-scrollbar,
      html[data-build-version="0.5.3"] #inventory::-webkit-scrollbar,
      html[data-build-version="0.5.3"] .equip-list::-webkit-scrollbar{width:9px!important;height:9px!important;}
      html[data-build-version="0.5.3"] .effect-scroll::-webkit-scrollbar-track,
      html[data-build-version="0.5.3"] .monster-record-list::-webkit-scrollbar-track,
      html[data-build-version="0.5.3"] #inventory::-webkit-scrollbar-track,
      html[data-build-version="0.5.3"] .equip-list::-webkit-scrollbar-track{background:#100905!important;border-radius:9px!important;}
      html[data-build-version="0.5.3"] .effect-scroll::-webkit-scrollbar-thumb,
      html[data-build-version="0.5.3"] .monster-record-list::-webkit-scrollbar-thumb,
      html[data-build-version="0.5.3"] #inventory::-webkit-scrollbar-thumb,
      html[data-build-version="0.5.3"] .equip-list::-webkit-scrollbar-thumb{background:linear-gradient(#d7a340,#7b5519)!important;border-radius:9px!important;border:1px solid #2b1a07!important;}

      html[data-build-version="0.5.3"] .equip-list{
        flex:1 1 auto!important;
        min-height:0!important;
        width:100%!important;
        max-width:none!important;
      ...
      html[data-build-version="0.5.3"] .equip-actions-fixed,
      html[data-build-version="0.5.3"] #bestEquipBtnEquip,
      html[data-build-version="0.5.3"] #upgradeBtn{display:none!important;}
      html[data-build-version="0.5.3"] .equip-left>#bestEquipBtn{flex:0 0 auto!important;display:block!important;width:100%!important;margin:8px 0 0!important;}

      html[data-build-version="0.5.3"] .inventory-panel.active-page{
        display:flex!important;
        flex-direction:column!important;
        width:100%!important;
        height:100%!important;
      ...
      html[data-build-version="0.5.3"] .inventory-panel h2{flex:0 0 auto!important;}
      html[data-build-version="0.5.3"] .inventory-actions,
      html[data-build-version="0.5.3"] .inventory-filter-bar{flex:0 0 auto!important;width:100%!important;max-width:none!important;}
      html[data-build-version="0.5.3"] #inventory{
        flex:1 1 auto!important;
        min-height:0!important;
        width:100%!important;
        max-width:none!important;
      ...
      html[data-build-version="0.5.3"] .menu-footer-fixed,
      html[data-build-version="0.5.3"] .side-panel>.legal-links,
      html[data-build-version="0.5.3"] .side-panel>.menu-footer{
        flex:0 0 auto!important;
        display:grid!important;
        grid-template-columns:repeat(3,minmax(0,1fr))!important;
        gap:8px!important;
      ...
      html[data-build-version="0.5.3"] .panel .legal-links,
      html[data-build-version="0.5.3"] .panel .menu-footer{display:none!important;}

      html[data-build-version="0.5.3"] body.menu-open .flee-btn,
      html[data-build-version="0.5.3"] body.menu-open .expbar{display:none!important;}

      @media(min-width:1280px) and (min-height:701px) and (pointer:fine){
        html[data-build-version="0.5.3"] .layout{grid-template-columns:minmax(780px,1fr) minmax(560px,760px)!important;}
        html[data-build-version="0.5.3"] .side-panel{position:static!important;display:grid!important;width:auto!important;height:auto!important;background:none!important;border:0!important;padding:0!important;box-shadow:none!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;}
        html[data-build-version="0.5.3"] #equipToggleBtn{display:none!important;}
      }
      @media(max-width:1279px),(max-height:700px),(pointer:coarse){
        html[data-build-version="0.5.3"] .layout{grid-template-columns:1fr!important;}
        html[data-build-version="0.5.3"] #equipToggleBtn{display:inline-flex!important;}
        html[data-build-version="0.5.3"] body:not(.mbh-menu-open) .side-panel{display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important;}
        html[data-build-version="0.5.3"] body.mbh-menu-open .side-panel{display:grid!important;position:fixed!important;left:8px!important;right:8px!important;top:calc(var(--mbh-topbar-h,58px) + 8px)!important;bottom:8px!important;width:auto!important;height:auto!important;z-index:70!important;background:rgba(0,0,0,.94)!important;border:2px solid #c99b39!important;padding:8px!important;box-shadow:0 0 35px #000!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;}
      }
      @media(max-width:760px){
        html[data-build-version="0.5.3"] .status-layout,
        html[data-build-version="0.5.3"] .equip-layout{grid-template-columns:1fr!important;overflow-y:auto!important;}
        html[data-build-version="0.5.3"] .side-panel>.mobile-menu-tabs{grid-template-columns:repeat(3,minmax(0,1fr))!important;}
        html[data-build-version="0.5.3"] .side-panel>.legal-links,
        html[data-build-version="0.5.3"] .side-panel>.menu-footer{grid-template-columns:1fr!important;}
        html[data-build-version="0.5.3"] .equip-list{grid-template-columns:1fr!important;}
        html[data-build-version="0.5.3"] #inventory{grid-template-columns:repeat(auto-fill,minmax(120px,1fr))!important;}
      }
    `;
  }

  function menuPage(){
    let p=(typeof state!=='undefined' && state.menuPage) ? state.menuPage : 'stats';
    return ['stats','equip','inventory'].includes(p) ? p : 'stats';
  }

  function normalizeMenu041Final(){
    const side=document.querySelector('.side-panel'); if(!side) return;
    const tabs=document.querySelector('.mobile-menu-tabs.side-tabs') || document.querySelector('.mobile-menu-tabs');
    if(tabs){
      $$('[data-menu-page="log"]',tabs).forEach(b=>b.remove());
      $$('[data-menu-page="equip"]',tabs).forEach(b=>{ b.textContent='装備'; });
      if(tabs.parentNode!==side) side.insertBefore(tabs,side.firstChild);
    }
    let content=$('mbhMenuContent041') || $('mbhMenuContent040') || $('mbhMenuContent039');
    if(!content){ content=document.createElement('div'); }
    content.id='mbhMenuContent041'; content.className='menu-content-fixed';
    ['.hero-stats','.equip-panel','.inventory-panel'].forEach(sel=>{ const p=document.querySelector(sel); if(p && p.parentNode!==content) content.appendChild(p); });
    $$('.log-panel').forEach(x=>x.remove());
    let footer=$('mbhMenuFooter041') || $('mbhMenuFooter040') || $('mbhMenuFooter039') || side.querySelector(':scope > .legal-links, :scope > .menu-footer');
    if(!footer){ footer=document.createElement('div'); footer.innerHTML='<button id="creditBtn" type="button">クレジット</button><button id="termsBtn" type="button">利用規約</button><button id="privacyBtn" type="button">プライバシーポリシー</button>'; }
    footer.id='mbhMenuFooter041'; footer.className='legal-links menu-footer-fixed'; footer.setAttribute('aria-label','公開情報');
    $$('.legal-links,.menu-footer').forEach(x=>{ if(x!==footer && x.closest('.side-panel')){ if(!footer.innerHTML.trim()) footer.innerHTML=x.innerHTML; x.remove(); } });
    if(!footer.innerHTML.trim()) footer.innerHTML='<button id="creditBtn" type="button">クレジット</button><button id="termsBtn" type="button">利用規約</button><button id="privacyBtn" type="button">プライバシーポリシー</button>';
    if(content.parentNode!==side) side.insertBefore(content, footer.parentNode===side?footer:null);
    if(footer.parentNode!==side) side.appendChild(footer);
    $$('.menu-col,.menu-main',side).forEach(x=>{ if(!x.querySelector('.panel')) x.remove(); });
    bindLegal041Final();
    setMenuPage041Final(menuPage());
  }

  function setMenuPage041Final(page){
    if(!['stats','equip','inventory'].includes(page)) page='stats';
    ...
    const target=document.querySelector(map[page]); if(target) target.classList.add('active-page');
    $$('[data-menu-page]').forEach(b=>b.classList.toggle('active',b.dataset.menuPage===page));
  }

  function bindTabs041Final(){
    $$('[data-menu-page]').forEach(b=>{
      if(b.dataset.menuPage==='log'){ b.remove(); return; }
      b.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); setMenuPage041Final(b.dataset.menuPage); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); return false; };
      b.ontouchend=b.onclick;
    });
  }

  function bindLegal041Final(){
    const bind=(id,type)=>{ const b=$(id); if(!b) return; b.onclick=(e)=>{ e.preventDefault(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal(type); }); }; };
    bind('creditBtn','credit'); bind('termsBtn','terms'); bind('privacyBtn','privacy');
  }

  function ensureEquipButton041Final(){
    const left=document.querySelector('.equip-left'); if(!left) return;
    $$('[id="bestEquipBtnEquip"],.equip-actions-fixed').forEach(x=>x.remove());
    let best=$('bestEquipBtn');
    if(!best){ best=document.createElement('button'); best.id='bestEquipBtn'; best.type='button'; best.textContent='最強装備'; }
    best.classList.add('wide');
    if(best.parentNode!==left) left.appendChild(best);
    best.onclick=(e)=>{ e.preventDefault(); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); safe(()=>{ if(typeof bestEquip==='function') bestEquip(); }); };
    const up=$('upgradeBtn'); if(up){ up.style.display='none'; up.disabled=true; up.classList.add('hidden'); }
  }

  function patchMenuButton041Final(){
    const btn=$('equipToggleBtn'); if(!btn) return;
    btn.textContent='メニュー';
    btn.onclick=(e)=>{
      e.preventDefault(); e.stopPropagation();
      const compact=matchMedia('(max-width:1279px), (max-height:700px), (pointer:coarse)').matches;
      if(!compact) return false;
      const open=!document.body.classList.contains('mbh-menu-open');
      document.body.classList.toggle('mbh-menu-open',open);
      document.body.classList.toggle('menu-open',open);
      const side=document.querySelector('.side-panel'); if(side) side.classList.toggle('open',open);
      if(typeof state!=='undefined') state.uiOpen=open;
      btn.textContent='メニュー';
      safe(()=>{ if(typeof playUiClick==='function') playUiClick(); });
      return false;
  ...
  function fitNoInline041Final(){
    ['inventory','log'].forEach(id=>{ const el=$(id); if(el){ el.style.maxHeight=''; el.style.height=''; el.style.overflow=''; } });
  }

  function boot041Final(){
    syncVersion041Final();
    installCss041Final();
    normalizeMenu041Final();
    bindTabs041Final();
    ensureEquipButton041Final();
    patchMenuButton041Final();
    fitNoInline041Final();
    setMenuPage041Final(menuPage());
  }

  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot041Final,{once:true}); else setTimeout(boot041Final,0);
  window.addEventListener('load',boot041Final,{once:true});
  window.addEventListener('resize',()=>setTimeout(boot041Final,0));
  setInterval(()=>safe(()=>{ syncVersion041Final(); normalizeMenu041Final(); ensureEquipButton041Final(); fitNoInline041Final(); }),1500);
})();

/* MBH ver.0.5.3: 固有能力表示の固定化 + レアリティ色付け + 戦闘中の一時効果を表示しない */
(function(){
  'use strict';
  const BUILD='0.5.3';
  ...
  const ABILITY_KEYS=new Set([
    'fireRes','fireDamageHeal','fireDmg','fireSkillChance','thunderDmg','thunderSkillChance',
    'deathDanceChance','deathDanceDefIgnore','heroDarkBleedChance','lifeSteal','guard','crit',
    'darkShield','darkAmulet','masterRegen','killHeal','skill'
  ]);
  ...
  const LABEL={
    ...
    fireRes:'火軽減', fireDamageHeal:'火吸収', fireDmg:'火ダメージ', fireSkillChance:'炎斬り発動率',
    thunderDmg:'雷ダメージ', thunderSkillChance:'雷撃発動率',
    deathDanceChance:'死線の剣舞率', deathDanceDefIgnore:'剣舞防御無視', heroDarkBleedChance:'暗黒出血付与',
    lifeSteal:'HP吸収', guard:'ガード率', crit:'クリティカル率', killHeal:'撃破時HP回復'
  };
  const DARK_EFFECT_LABEL={
    darkShield:['闇の盾','毎ターン被ダメ軽減+1%（最大50%） / 被ダメージ50%回復'],
    darkAmulet:['闇のアミュレット','死線の剣舞効果時間2倍'],
    masterRegen:['師匠のアミュレット','10秒ごとにHP回復 / 撃破時HP25%回復']
  };
  function safe(fn){ try{return fn&&fn();}catch(e){ console.warn('[MBH 0.5.3]', e); return null; } }
  function $(id){return document.getElementById(id);}
  ...
  function colorOf(it){
    ...
    return ({legendary:'#ff9b24',rare:'#4d8dff',normal:'#eeeeee'}[it.rarity] || '#eeeeee');
  }
  ...
  function addAbilityRowsFromItem(rows,it,aggregate=false){
    const c=colorOf(it);
    const add=(key,val,label=LABEL[key])=>{ if(Number(val||0)) rows.push({label,value:pct(val,true),color:c,key,ability:true}); };
    add('fireRes',it.fireRes,LABEL.fireRes);
    add('fireDamageHeal',it.fireDamageHeal,LABEL.fireDamageHeal);
    add('fireDmg',it.fireDmg,LABEL.fireDmg);
    add('fireSkillChance',it.fireSkillChance,LABEL.fireSkillChance);
    add('thunderDmg',it.thunderDmg,LABEL.thunderDmg);
    add('thunderSkillChance',it.thunderSkillChance,LABEL.thunderSkillChance);
    add('deathDanceChance',it.deathDanceChance,LABEL.deathDanceChance);
    add('deathDanceDefIgnore',it.deathDanceDefIgnore,LABEL.deathDanceDefIgnore);
    add('heroDarkBleedChance',it.heroDarkBleedChance,LABEL.heroDarkBleedChance);
    add('lifeSteal',it.lifeSteal,LABEL.lifeSteal);
    add('guard',it.guard,LABEL.guard);
    add('crit',it.crit,LABEL.crit);
    add('killHeal',it.killHeal,LABEL.killHeal);
    if(it.darkShield){ const [l,v]=DARK_EFFECT_LABEL.darkShield; rows.push({label:l,value:v,color:DARK_COLOR,key:'darkShield',ability:true}); }
    if(it.darkAmulet){ const [l,v]=DARK_EFFECT_LABEL.darkAmulet; rows.push({label:l,value:v,color:DARK_COLOR,key:'darkAmulet',ability:true}); }
    if(it.masterRegen){ const [l,v]=DARK_EFFECT_LABEL.masterRegen; rows.push({label:l,value:v,color:c,key:'masterRegen',ability:true}); }
    // 闇の兜・闇の籠手など、現時点で専用効果がコード実装されていないものは固有効果として表示しない。
    // HP/攻撃/防御などの基礎パラメータは装備効果合計側にだけ出す。
    if(it.skill && it.skill.name){
      ...
      rows.push({label:'武器スキル',value:`${it.skill.name} ${pct(chance,false)}`,color:c,key:'skill',ability:true});
  ...
  function mergeRows(rows){
    ...
    rows.forEach(r=>{
      ...
      if(PCT_KEYS.has(id)){
        ...
        if(!old){ map.set(id,{...r,_sum:val}); out.push(map.get(id)); }
        else{ old._sum+=val; old.value=pct(old._sum,true); if(priority(r.color)>priority(old.color)) old.color=r.color; }
      }else{
        if(!map.has(id)){ map.set(id,{...r}); out.push(map.get(id)); }
      }
    });
    return out;
  }
  function abilityRows(){ const rows=[]; itemList().forEach(it=>addAbilityRowsFromItem(rows,it)); return mergeRows(rows); }
  function equipmentRows(){
    ...
    if(t.hp) rows.push({label:'HP',value:num(t.hp,true),color:BASIC_COLOR,basic:true});
    if(t.atk) rows.push({label:'攻撃力',value:num(t.atk,true),color:BASIC_COLOR,basic:true});
    if(t.def) rows.push({label:'防御力',value:num(t.def,true),color:BASIC_COLOR,basic:true});
  ...
  let applying=false, lastHash='';
  function renderCanonicalEffects(force=false){
    if(applying) return;
    const h=itemHash();
    ...
    const shtml=rowsHtml(abilityRows(),'特殊効果なし');
    const ehtml=rowsHtml(equipmentRows(),'装備効果なし');
    applying=true;
    setIfChanged(sbox,shtml);
    setIfChanged(ebox,ehtml);
    lastHash=h;
    applying=false;
  }
  function installObserver(){
    ['#statusSpecialEffects .effect-scroll','#equipEffectTotals .effect-scroll'].forEach(sel=>{
      const el=document.querySelector(sel); if(!el || el.__mbh042Observed) return;
      el.__mbh042Observed=true;
      new MutationObserver(()=>{ if(!applying) requestAnimationFrame(()=>renderCanonicalEffects(true)); }).observe(el,{childList:true,subtree:true,characterData:true});
    });
  }
  function installCss(){
    ...
    st.textContent=`
      html[data-build-version="0.5.3"] .effect-row.ability-option span,
      html[data-build-version="0.5.3"] .effect-row.ability-option b{color:var(--effect-color)!important;text-shadow:0 0 8px color-mix(in srgb, var(--effect-color) 55%, transparent)!important;}
      html[data-build-version="0.5.3"] .effect-row.basic-option span{color:#e8d0a2!important;}
      html[data-build-version="0.5.3"] .effect-row.basic-option b{color:#fff7c8!important;}
      html[data-build-version="0.5.3"] .equip small .item-ability-option{color:var(--item-ability-color)!important;text-shadow:0 0 7px color-mix(in srgb, var(--item-ability-color) 50%, transparent)!important;font-weight:800!important;}
      html[data-build-version="0.5.3"] .equip small .item-basic-option{color:#c8b98e!important;}
    `;
    document.head.appendChild(st);
  }
  function coloredSummaryHtml(it){
    const parts=[]; const basic=(s)=>parts.push(`<span class="item-basic-option">${esc(s)}</span>`); const ability=(s)=>parts.push(`<span class="item-ability-option" style="--item-ability-color:${esc(colorOf(it))}">${esc(s)}</span>`);
    if(it.itemLevel) basic(`ドロップLv${Math.floor(Number(it.itemLevel)||1)}`);
    basic(`品質+${Math.max(0,Math.floor(Number(it.level)||0))}`);
    if(it.atk) basic(`攻+${Math.floor(Number(it.atk)||0)}`); if(it.def) basic(`防+${Math.floor(Number(it.def)||0)}`); if(it.hp) basic(`HP+${Math.floor(Number(it.hp)||0)}`);
    if(it.fireRes) ability(`火軽減${pct(it.fireRes,false)}`);
    if(it.fireDamageHeal) ability(`火吸収${pct(it.fireDamageHeal,false)}`);
    if(it.fireDmg) ability(`火ダメージ${pct(it.fireDmg,true)}`);
    if(it.fireSkillChance) ability(`炎斬り発動率${pct(it.fireSkillChance,true)}`);
    if(it.thunderDmg) ability(`雷ダメージ${pct(it.thunderDmg,true)}`);
    if(it.thunderSkillChance) ability(`雷撃発動率${pct(it.thunderSkillChance,true)}`);
    if(it.deathDanceChance) ability(`死線の剣舞率${pct(it.deathDanceChance,true)}`);
    if(it.deathDanceDefIgnore) ability(`剣舞防御無視${pct(it.deathDanceDefIgnore,false)}`);
    if(it.heroDarkBleedChance) ability(`暗黒出血付与${pct(it.heroDarkBleedChance,false)}`);
    if(it.lifeSteal) ability(`HP吸収${pct(it.lifeSteal,false)}`);
    if(it.guard) ability(`ガード率${pct(it.guard,true)}`);
    if(it.crit) ability(`クリティカル率${pct(it.crit,true)}`);
    if(it.darkShield) ability(DARK_EFFECT_LABEL.darkShield[1]);
    if(it.darkAmulet) ability(DARK_EFFECT_LABEL.darkAmulet[1]);
    if(it.masterRegen) ability(DARK_EFFECT_LABEL.masterRegen[1]);
    if(it.skill && it.skill.name){ const extra=(it.skill.id==='fire'?(Number(it.fireSkillChance)||0):it.skill.id==='thunder'?(Number(it.thunderSkillChance)||0):0); ability(`武器スキル：${it.skill.name} ${pct((Number(it.skill.chance)||0)+extra,false)}`); }
    return parts.join(' / ');
  }
  function patchItemSummary(){
    safe(()=>{ itemSummary=function(it){ return coloredSummaryHtml(it); }; });
  }
  function patchRenderEquip(){
    if(typeof renderEquip!=='function' || renderEquip.__mbh042) return;
    const old=renderEquip;
    renderEquip=function(){ const r=old.apply(this,arguments); safe(()=>{ renderCanonicalEffects(true); installObserver(); }); return r; };
    renderEquip.__mbh042=true;
  }
  function patchRenderStats(){
    if(typeof renderStats!=='function' || renderStats.__mbh042) return;
    const old=renderStats;
    renderStats=function(){ const r=old.apply(this,arguments); safe(()=>{ renderCanonicalEffects(false); installObserver(); }); return r; };
    renderStats.__mbh042=true;
  }
  function syncVersion(){
    window.APP_VERSION=BUILD; window.GAME_VERSION=BUILD;
    document.documentElement.dataset.buildVersion=BUILD;
  ...
  function boot(){
    syncVersion(); installCss(); patchItemSummary(); patchRenderEquip(); patchRenderStats(); renderCanonicalEffects(true); installObserver();
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot,{once:true}); else setTimeout(boot,0);
  window.addEventListener('load',boot,{once:true});
...
/* MBH ver.0.5.3: version single-source, stable effect panels, debug/menu layout lock, named regen display */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const BASIC_COLOR='#f7ead0';
  ...
  function safe(fn){ try{return fn&&fn();}catch(e){ console.warn('[MBH0.5.3]', e); return null; } }
  function $(id){ return document.getElementById(id); }
  function esc(s){ return String(s??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])); }
  function pct(v,signed=false){ const n=Math.round(Number(v||0)*100); return (signed&&n>0?'+':'')+n+'%'; }
  function num(v,signed=true){ const n=Math.floor(Number(v||0)); return (signed&&n>0?'+':'')+n.toLocaleString(); }
  ...
  function installCss(){
    ...
    st.textContent=`
      ...
      html[data-build-version="0.5.3"] .effect-row.basic-option span{color:#e8d0a2!important;}
      html[data-build-version="0.5.3"] .effect-row.basic-option b{color:#fff7c8!important;}
      html[data-build-version="0.5.3"] #statusSpecialEffects .effect-scroll,
      html[data-build-version="0.5.3"] #equipEffectTotals .effect-scroll{scrollbar-color:#9d742a #111!important;scrollbar-width:thin!important;overflow:auto!important;}
      html[data-build-version="0.5.3"] #statusSpecialEffects .effect-scroll::-webkit-scrollbar,
      html[data-build-version="0.5.3"] #equipEffectTotals .effect-scroll::-webkit-scrollbar{width:8px!important;}
      html[data-build-version="0.5.3"] #statusSpecialEffects .effect-scroll::-webkit-scrollbar-thumb,
      html[data-build-version="0.5.3"] #equipEffectTotals .effect-scroll::-webkit-scrollbar-thumb{background:#8b6628!important;border-radius:8px!important;}
      html[data-build-version="0.5.3"] #statusSpecialEffects .effect-scroll::-webkit-scrollbar-track,
      html[data-build-version="0.5.3"] #equipEffectTotals .effect-scroll::-webkit-scrollbar-track{background:#120b06!important;}
      @media (min-width:1280px) and (min-height:701px){
        html[data-build-version="0.5.3"] .side-panel{position:static!important;display:grid!important;grid-template-rows:auto minmax(0,1fr) auto!important;grid-template-columns:1fr!important;overflow:hidden!important;}
        html[data-build-version="0.5.3"] .side-panel .mobile-menu-tabs{grid-row:1!important;position:relative!important;top:auto!important;left:auto!important;right:auto!important;display:flex!important;z-index:2!important;}
        html[data-build-version="0.5.3"] #debugPanel:not(.hidden){position:fixed!important;right:18px!important;top:70px!important;z-index:12000!important;max-height:calc(100vh - 88px)!important;overflow:auto!important;}
        html[data-build-version="0.5.3"] #debugPanel:not(.hidden) ~ *{position:static!important;}
      }
    `;
    document.head.appendChild(st);
  }
...
/* MBH ver.0.5.3: final menu full-page layout + debug next enemy queue + spawn overwrite guard */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(id)=>document.getElementById(id);
  const qs=(s,r=document)=>r.querySelector(s);
  const qsa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const safe=(fn)=>{ try{ return fn&&fn(); }catch(e){ console.warn('[MBH0.5.3]', e); return null; } };
  ...
  function ensureCss044(){
    ...
    st.textContent=`
      ...
      html[data-build-version="0.5.3"] .side-panel>.mobile-menu-tabs button{flex:1 1 0!important;min-width:0!important;white-space:nowrap!important;}
      html[data-build-version="0.5.3"] .side-panel>.menu-col,
      html[data-build-version="0.5.3"] .side-panel>.menu-main,
      html[data-build-version="0.5.3"] .side-panel>.menu-left,
      html[data-build-version="0.5.3"] .side-panel>.menu-right{display:none!important;}
      html[data-build-version="0.5.3"] .side-panel>.menu-content-fixed,
      html[data-build-version="0.5.3"] #mbhMenuContent044{
      ...
      html[data-build-version="0.5.3"] #mbhMenuContent044>.panel{
      ...
      html[data-build-version="0.5.3"] #mbhMenuContent044>.panel:not(.active-page){display:none!important;}
      html[data-build-version="0.5.3"] #mbhMenuContent044>.panel.active-page{display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.5.3"] .hero-stats>.status-layout,
      html[data-build-version="0.5.3"] .equip-panel>.equip-layout{
      ...
      html[data-build-version="0.5.3"] .status-right,
      html[data-build-version="0.5.3"] .equip-left,
      html[data-build-version="0.5.3"] .equip-right{min-width:0!important;min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.5.3"] .status-special-effects,
      html[data-build-version="0.5.3"] .monster-records,
      html[data-build-version="0.5.3"] .equip-effect-totals{flex:1 1 auto!important;min-height:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.5.3"] .effect-scroll,
      html[data-build-version="0.5.3"] .monster-record-list,
      html[data-build-version="0.5.3"] .equip-list,
      html[data-build-version="0.5.3"] .inventory{min-height:0!important;overflow-y:auto!important;overflow-x:hidden!important;scrollbar-color:#9d742a #111!important;scrollbar-width:thin!important;}
      html[data-build-version="0.5.3"] .effect-scroll::-webkit-scrollbar,
      html[data-build-version="0.5.3"] .monster-record-list::-webkit-scrollbar,
      html[data-build-version="0.5.3"] .equip-list::-webkit-scrollbar,
      html[data-build-version="0.5.3"] .inventory::-webkit-scrollbar{width:8px!important;}
      html[data-build-version="0.5.3"] .effect-scroll::-webkit-scrollbar-thumb,
      html[data-build-version="0.5.3"] .monster-record-list::-webkit-scrollbar-thumb,
      html[data-build-version="0.5.3"] .equip-list::-webkit-scrollbar-thumb,
      html[data-build-version="0.5.3"] .inventory::-webkit-scrollbar-thumb{background:#8b6628!important;border-radius:8px!important;}
      html[data-build-version="0.5.3"] .effect-scroll::-webkit-scrollbar-track,
      html[data-build-version="0.5.3"] .monster-record-list::-webkit-scrollbar-track,
      html[data-build-version="0.5.3"] .equip-list::-webkit-scrollbar-track,
      html[data-build-version="0.5.3"] .inventory::-webkit-scrollbar-track{background:#120b06!important;}
      html[data-build-version="0.5.3"] .inventory-panel.active-page{display:flex!important;flex-direction:column!important;}
      html[data-build-version="0.5.3"] .inventory-panel .inventory{flex:1 1 auto!important;width:100%!important;max-height:none!important;display:grid!important;grid-template-columns:repeat(auto-fill,minmax(96px,1fr))!important;align-content:start!important;}
      html[data-build-version="0.5.3"] .menu-footer{grid-row:3!important;position:relative!important;display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:8px!important;width:100%!important;margin:0!important;z-index:4!important;}
      html[data-build-version="0.5.3"] #bestEquipBtnEquip{display:none!important;}
      @media (min-width:1280px) and (min-height:701px){
        html[data-build-version="0.5.3"] #equipToggleBtn{display:none!important;}
        html[data-build-version="0.5.3"] .side-panel{position:static!important;width:auto!important;background:none!important;border:0!important;box-shadow:none!important;padding:0!important;}
        html[data-build-version="0.5.3"] #debugPanel:not(.hidden){position:fixed!important;right:18px!important;top:70px!important;z-index:12000!important;max-height:calc(100vh - 88px)!important;overflow:auto!important;}
      }
      @media (max-width:1279px),(max-height:700px){
        html[data-build-version="0.5.3"] #equipToggleBtn{display:inline-block!important;}
        html[data-build-version="0.5.3"] .side-panel{display:none!important;position:fixed!important;left:8px!important;right:8px!important;top:calc(var(--mbh-topbar-h,58px) + 8px)!important;bottom:8px!important;width:auto!important;max-width:calc(100vw - 16px)!important;z-index:70!important;background:rgba(0,0,0,.94)!important;border:2px solid #c99b39!important;padding:8px!important;box-shadow:0 0 35px #000!important;}
        html[data-build-version="0.5.3"] .side-panel.open{display:grid!important;}
      }
      @media (max-width:760px){
        html[data-build-version="0.5.3"] .hero-stats>.status-layout,
        html[data-build-version="0.5.3"] .equip-panel>.equip-layout{grid-template-columns:1fr!important;overflow-y:auto!important;}
        html[data-build-version="0.5.3"] .menu-footer{grid-template-columns:1fr!important;}
      }
    `;
    document.head.appendChild(st);
  }
...
/* MBH ver.0.5.3: final build lock + visible menu tabs repair */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(id)=>document.getElementById(id);
  ...
  function ensureMenu045(){
    const panel=qs('.side-panel'); if(!panel) return;
    let tabs=qs(':scope > .mobile-menu-tabs', panel) || qs(':scope > .side-tabs', panel);
    if(!tabs){
      tabs=document.createElement('nav'); tabs.className='mobile-menu-tabs side-tabs'; tabs.setAttribute('aria-label','右側タブ');
      panel.insertBefore(tabs, panel.firstChild);
    }
    ...
    let content=$('mbhMenuContent044');
    if(!content){ content=document.createElement('div'); content.id='mbhMenuContent044'; content.className='menu-content-fixed'; }
    const footer=qs(':scope > .menu-footer', panel) || qs(':scope > .legal-links', panel);
    if(content.parentElement!==panel){ if(footer) panel.insertBefore(content, footer); else panel.appendChild(content); }
    ['.hero-stats','.equip-panel','.inventory-panel'].forEach(sel=>{
      const el=qs(sel, panel) || qs(sel, document);
      if(el && el.parentElement!==content) content.appendChild(el);
    });
    qsa('.log-panel', panel).forEach(el=>el.remove());
    let legal=footer || qs('.legal-links.menu-footer', document);
    if(legal){ legal.className='legal-links menu-footer'; if(legal.parentElement!==panel) panel.appendChild(legal); else panel.appendChild(legal); }
    const page=(typeof state!=='undefined' && state.menuPage) || 'stats';
    setPage045(page);
  }
  function setPage045(page){
    page = (page==='equip'||page==='inventory') ? page : 'stats';
    if(typeof state!=='undefined') state.menuPage=page;
    const map={stats:'.hero-stats',equip:'.equip-panel',inventory:'.inventory-panel'};
    qsa('.side-panel>.mobile-menu-tabs button').forEach(b=>b.classList.toggle('active', b.dataset.menuPage===page));
    qsa('#mbhMenuContent044>.panel').forEach(p=>p.classList.remove('active-page'));
    const t=qs('#mbhMenuContent044>'+map[page]); if(t) t.classList.add('active-page');
  }
  function bind045(){
    qsa('.side-panel>.mobile-menu-tabs button').forEach(b=>{
      b.onclick=(e)=>{e.preventDefault();e.stopPropagation();safe(()=>{if(typeof playUiClick==='function')playUiClick();});setPage045(b.dataset.menuPage);safe(()=>{if(typeof scheduleSave==='function')scheduleSave();});return false;};
    });
    const panel=qs('.side-panel'), btn=$('equipToggleBtn');
    if(panel && btn){
      btn.onclick=(e)=>{e.preventDefault();e.stopPropagation();const compact=matchMedia('(max-width:1279px),(max-height:700px)').matches;if(!compact)return false;const open=!panel.classList.contains('open');panel.classList.toggle('open',open);if(typeof state!=='undefined')state.uiOpen=open;btn.textContent=open?'閉じる':'メニュー';ensureMenu045();return false;};
    }
  }
  function forceVisible045(){
    const tabs=qs('.side-panel>.mobile-menu-tabs');
    if(tabs){
      tabs.style.display='flex'; tabs.style.visibility='visible'; tabs.style.opacity='1'; tabs.style.height='auto'; tabs.style.minHeight='38px'; tabs.style.position='relative'; tabs.style.overflow='visible';
      qsa('button',tabs).forEach(b=>{ b.style.display='inline-flex'; b.style.minHeight='38px'; b.style.minWidth=matchMedia('(max-width:760px)').matches?'0':'96px'; b.style.flex='1 1 0'; b.style.visibility='visible'; b.style.opacity='1'; });
    }
  }
  function boot045(){syncVersion045();ensureMenu045();bind045();forceVisible045();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot045,{once:true}); else setTimeout(boot045,0);
  window.addEventListener('load',boot045,{once:true});
  window.addEventListener('resize',()=>setTimeout(boot045,30));
  setInterval(()=>safe(boot045),80);
})();


/* MBH ver.0.5.3: safe menu layer fix - no MutationObserver loop */
(function(){
  'use strict';
  const BUILD='0.5.3';
  ...
  function syncVersion047(){
    ...
    try{
      ...
      $$('.debug-version').forEach(el=>{ el.textContent='Build: ver.'+BUILD; });
      $$('.debug-trace-title').forEach(el=>{ el.textContent='進行デバッグログ ver.'+BUILD; });
    }catch(e){ console.warn('[MBH0.5.3 syncVersion]', e); }
    finally{ applying=false; }
  }
  function installCss047(){
    if(document.getElementById('mbh-047-safe-layer-css')) return;
    const st=document.createElement('style');
    st.id='mbh-047-safe-layer-css';
    st.textContent=`
      html[data-build-version="0.5.3"] .layout{position:relative!important;z-index:1!important;}
      html[data-build-version="0.5.3"] .battle-panel{position:relative!important;z-index:1!important;}
      html[data-build-version="0.5.3"] .side-panel{position:relative!important;z-index:9000!important;pointer-events:auto!important;}
      html[data-build-version="0.5.3"] .side-panel.open{position:fixed!important;z-index:20000!important;pointer-events:auto!important;}
      html[data-build-version="0.5.3"] #debugPanel:not(.hidden){z-index:21000!important;pointer-events:auto!important;}
      html[data-build-version="0.5.3"] .mobile-menu-tabs{display:flex!important;width:100%!important;min-height:40px!important;gap:8px!important;flex:0 0 auto!important;}
      html[data-build-version="0.5.3"] .mobile-menu-tabs button,
      html[data-build-version="0.5.3"] [data-menu-page]{display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:1 1 0!important;min-width:88px!important;min-height:40px!important;height:40px!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;white-space:nowrap!important;}
      html[data-build-version="0.5.3"] .menu-footer{position:relative!important;z-index:9001!important;pointer-events:auto!important;}
      html[data-build-version="0.5.3"] .menu-footer button{min-width:0!important;pointer-events:auto!important;}
      @media (min-width:1280px) and (min-height:701px){
        html[data-build-version="0.5.3"] .side-panel{display:grid!important;position:relative!important;grid-column:2!important;width:100%!important;height:100%!important;background:none!important;border:0!important;box-shadow:none!important;padding:0!important;}
      }
      @media (max-width:1279px),(max-height:700px){
        html[data-build-version="0.5.3"] .side-panel:not(.open){display:none!important;pointer-events:none!important;}
        html[data-build-version="0.5.3"] .side-panel.open{display:grid!important;left:8px!important;right:8px!important;top:calc(var(--mbh-topbar-h,58px) + 8px)!important;bottom:8px!important;width:auto!important;}
      }
    `;
    document.head.appendChild(st);
  }
  function boot047(){ syncVersion047(); installCss047(); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot047, {once:true}); else boot047();
  window.addEventListener('load', boot047, {once:true});
  window.addEventListener('resize', ()=>setTimeout(boot047, 50));
  window.addEventListener('orientationchange', ()=>setTimeout(boot047, 100));
  setTimeout(boot047, 100);
  setTimeout(boot047, 500);
})();


/* ver.0.5.3: final menu DOM/geometry guard. No observer, no BGM assets. */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const safe=(fn)=>{ try{return fn&&fn();}catch(e){ console.error('[MBH 0.5.3 menu]', e); } };
  ...
  function panelFor(page){
    return page==='equip' ? $('.equip-panel') : page==='inventory' ? $('.inventory-panel') : $('.hero-stats');
  }
  function ensureMenuContent049(){
    const side=$('.side-panel'); if(!side) return null;
    let tabs=side.querySelector(':scope > .mobile-menu-tabs, :scope > .side-tabs') || $('.mobile-menu-tabs');
    if(tabs && tabs.parentElement!==side) side.insertBefore(tabs, side.firstChild);
    let content=document.getElementById('mbhMenuContent044') || document.getElementById('mbhMenuContent049') || side.querySelector(':scope > .menu-content-fixed');
    if(!content){ content=document.createElement('div'); content.id='mbhMenuContent044'; content.className='menu-content-fixed'; }
    ...
    ['.hero-stats','.equip-panel','.inventory-panel'].forEach(sel=>{
      const p=$(sel); if(p && p.parentElement!==content) content.appendChild(p);
    });
    ...
    $$('.menu-col,.menu-left,.menu-right', side).forEach(el=>{ if(!el.children.length) el.remove(); });
    return content;
  }
  function setPage049(page){
    if(typeof state!=='undefined') state.menuPage=page||state.menuPage||'stats';
    const current=(typeof state!=='undefined' && state.menuPage) || page || 'stats';
    ensureMenuContent049();
    $$('[data-menu-page]').forEach(btn=>btn.classList.toggle('active', btn.dataset.menuPage===current));
    $$('.side-panel .panel').forEach(p=>p.classList.remove('active-page'));
    const target=panelFor(current) || panelFor('stats');
    if(target) target.classList.add('active-page');
  }
  function isCompact049(){ return window.matchMedia('(max-width:1279px), (max-height:700px)').matches; }
  function applyMenuGeometry049(){
    const side=$('.side-panel'); const btn=$('#equipToggleBtn'); if(!side) return;
    ensureMenuContent049();
    if(isCompact049()){
      if(btn){ btn.style.display='inline-flex'; btn.textContent=side.classList.contains('open')?'閉じる':'メニュー'; }
    }else{
      side.classList.remove('open');
    ...
    setPage049((typeof state!=='undefined' && state.menuPage) || 'stats');
  }
  function bindMenu049(){
    const side=$('.side-panel'); const btn=$('#equipToggleBtn'); if(!side) return;
    $$('[data-menu-page]').forEach(b=>{
      if(b.__mbh049Bound) return; b.__mbh049Bound=true;
      b.addEventListener('click', e=>{ e.preventDefault(); e.stopPropagation(); setPage049(b.dataset.menuPage||'stats'); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); }, true);
      b.addEventListener('pointerup', e=>{ e.preventDefault(); e.stopPropagation(); setPage049(b.dataset.menuPage||'stats'); }, true);
    });
    if(btn && !btn.__mbh049Bound){
      btn.__mbh049Bound=true;
      btn.addEventListener('click', e=>{
        e.preventDefault(); e.stopPropagation();
        if(!isCompact049()) return;
        side.classList.toggle('open');
        if(typeof state!=='undefined') state.uiOpen=side.classList.contains('open');
        applyMenuGeometry049();
        safe(()=>{ if(typeof startAudio==='function') startAudio(); if(typeof playUiClick==='function') playUiClick(); });
      }, true);
    }
  }
  function boot049(){ syncVersion049(); ensureMenuContent049(); bindMenu049(); applyMenuGeometry049(); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot049, {once:true}); else setTimeout(boot049,0);
  window.addEventListener('load', boot049, {once:true});
  window.addEventListener('resize', ()=>setTimeout(applyMenuGeometry049,30));
  setTimeout(boot049,250);
  setTimeout(boot049,1000);
})();


/* MBH ver.0.5.3: verified menu geometry finalizer. No MutationObserver, no BGM assets. */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const safe=(fn)=>{ try{ return fn&&fn(); }catch(e){ console.error('[MBH 0.5.3 menu]', e); return null; } };
  ...
  function installRuntimeCss050(){
    ...
    st.textContent = `
      html[data-build-version="0.5.3"] .side-panel{grid-template-columns:minmax(0,1fr)!important;grid-template-rows:auto minmax(0,1fr) auto!important;align-items:stretch!important;justify-items:stretch!important;width:100%!important;min-width:0!important;overflow:hidden!important;z-index:30000!important;pointer-events:auto!important;}
      html[data-build-version="0.5.3"] .side-panel>*{grid-column:1 / -1!important;width:100%!important;min-width:0!important;max-width:none!important;box-sizing:border-box!important;}
      html[data-build-version="0.5.3"] .side-panel>.mobile-menu-tabs,html[data-build-version="0.5.3"] .side-panel>.side-tabs{grid-row:1!important;display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:8px!important;width:100%!important;min-width:0!important;height:44px!important;min-height:44px!important;max-height:44px!important;overflow:visible!important;position:relative!important;z-index:30002!important;}
      html[data-build-version="0.5.3"] [data-menu-page]{display:flex!important;align-items:center!important;justify-content:center!important;width:100%!important;min-width:0!important;max-width:none!important;height:44px!important;min-height:44px!important;max-height:44px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;pointer-events:auto!important;visibility:visible!important;opacity:1!important;}
      html[data-build-version="0.5.3"] #mbhMenuContent050,html[data-build-version="0.5.3"] #mbhMenuContent044,html[data-build-version="0.5.3"] #mbhMenuContent041,html[data-build-version="0.5.3"] .side-panel>.menu-content-fixed{grid-row:2!important;display:block!important;width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;overflow:hidden!important;position:relative!important;z-index:30001!important;}
      html[data-build-version="0.5.3"] .menu-footer{grid-row:3!important;display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:8px!important;width:100%!important;min-width:0!important;position:relative!important;z-index:30002!important;pointer-events:auto!important;}
      html[data-build-version="0.5.3"] .menu-footer button{width:100%!important;min-width:0!important;pointer-events:auto!important;}
    `;
  }
  function getPage050(){ return (typeof state!=='undefined' && state.menuPage) || 'stats'; }
  function panelFor050(page){ return page==='equip' ? $('.equip-panel') : page==='inventory' ? $('.inventory-panel') : $('.hero-stats'); }
  function ensureMenu050(){
    const side=$('.side-panel'); if(!side) return null;
    side.style.gridTemplateColumns='minmax(0, 1fr)';
    side.style.gridTemplateRows='auto minmax(0, 1fr) auto';
    side.style.alignItems='stretch'; side.style.justifyItems='stretch'; side.style.pointerEvents='auto'; side.style.overflow='hidden';
    let tabs=side.querySelector(':scope > .mobile-menu-tabs, :scope > .side-tabs') || $('.mobile-menu-tabs,.side-tabs');
    if(tabs && tabs.parentElement!==side) side.insertBefore(tabs, side.firstChild);
    if(tabs){
      tabs.style.gridColumn='1 / -1'; tabs.style.gridRow='1'; tabs.style.display='grid'; tabs.style.gridTemplateColumns='repeat(3,minmax(0,1fr))'; tabs.style.width='100%'; tabs.style.minWidth='0'; tabs.style.height='44px'; tabs.style.pointerEvents='auto';
    }
    let content=document.getElementById('mbhMenuContent050') || document.getElementById('mbhMenuContent044') || document.getElementById('mbhMenuContent041') || side.querySelector(':scope > .menu-content-fixed');
    if(!content){ content=document.createElement('div'); content.id='mbhMenuContent050'; content.className='menu-content-fixed'; }
    content.id='mbhMenuContent050';
    content.classList.add('menu-content-fixed');
    if(content.parentElement!==side){
      const footer=side.querySelector(':scope > .menu-footer, :scope > .legal-links');
      side.insertBefore(content, footer || null);
    }
    content.style.gridColumn='1 / -1'; content.style.gridRow='2'; content.style.width='100%'; content.style.height='100%'; content.style.minWidth='0'; content.style.minHeight='0'; content.style.overflow='hidden'; content.style.position='relative';
    ['.hero-stats','.equip-panel','.inventory-panel'].forEach(sel=>{ const p=$(sel); if(p && p.parentElement!==content) content.appendChild(p); });
    let footer=side.querySelector(':scope > .menu-footer, :scope > .legal-links') || $('.menu-footer,.legal-links');
    if(footer){ footer.classList.add('menu-footer'); if(footer.parentElement!==side) side.appendChild(footer); footer.style.gridColumn='1 / -1'; footer.style.gridRow='3'; footer.style.width='100%'; footer.style.minWidth='0'; footer.style.display='grid'; footer.style.gridTemplateColumns='repeat(3,minmax(0,1fr))'; }
    $$('.menu-col,.menu-left,.menu-right', side).forEach(el=>{ if(!el.children.length) el.remove(); else el.style.display='none'; });
    $$('[data-menu-page]').forEach(b=>{ b.style.width='100%'; b.style.minWidth='0'; b.style.height='44px'; b.style.pointerEvents='auto'; b.style.visibility='visible'; b.style.opacity='1'; });
    return content;
  }
  function setPage050(page){
    const current=page || getPage050();
    if(typeof state!=='undefined') state.menuPage=current;
    ensureMenu050();
    $$('[data-menu-page]').forEach(btn=>btn.classList.toggle('active', btn.dataset.menuPage===current));
    $$('.side-panel .panel').forEach(p=>p.classList.remove('active-page'));
    const target=panelFor050(current) || panelFor050('stats');
    if(target) target.classList.add('active-page');
  }
  function compact050(){ return window.matchMedia('(max-width:1279px), (max-height:700px)').matches; }
  function apply050(){
    syncVersion050(); installRuntimeCss050();
    const side=$('.side-panel'); const btn=$('#equipToggleBtn'); if(!side) return;
    ensureMenu050();
    if(compact050()){
      if(btn){ btn.style.display='inline-flex'; btn.textContent=side.classList.contains('open')?'閉じる':'メニュー'; }
    }else{
      side.classList.remove('open'); side.style.display='grid'; side.style.position='relative'; side.style.zIndex='30000';
      if(btn){ btn.style.display='none'; btn.textContent='メニュー'; }
      if(typeof state!=='undefined') state.uiOpen=false;
    }
    setPage050(getPage050());
  }
  function bind050(){
    $$('[data-menu-page]').forEach(b=>{
      if(b.__mbh050Bound) return; b.__mbh050Bound=true;
      const handler=(e)=>{ e.preventDefault(); e.stopPropagation(); setPage050(b.dataset.menuPage||'stats'); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); return false; };
      b.addEventListener('click', handler, true); b.addEventListener('pointerup', handler, true);
    });
    const btn=$('#equipToggleBtn'); const side=$('.side-panel');
    if(btn && side && !btn.__mbh050Bound){ btn.__mbh050Bound=true; btn.addEventListener('click', e=>{ e.preventDefault(); e.stopPropagation(); if(!compact050()) return false; side.classList.toggle('open'); if(typeof state!=='undefined') state.uiOpen=side.classList.contains('open'); apply050(); return false; }, true); }
  }
  function boot050(){ apply050(); bind050(); setTimeout(apply050,50); setTimeout(apply050,300); setTimeout(apply050,1000); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot050, {once:true}); else setTimeout(boot050,0);
  window.addEventListener('load', boot050, {once:true});
  window.addEventListener('resize', ()=>setTimeout(apply050,50));
})();

/* MBH ver.0.5.3: click routing repair for fixed right menu. No BGM assets. */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const safe=(fn)=>{ try{return fn&&fn();}catch(e){ console.error('[MBH0.5.3]', e); return null; } };
  ...
  function panelFor(page){ return page==='equip' ? $('.equip-panel') : page==='inventory' ? $('.inventory-panel') : $('.hero-stats'); }
  function ensureMenu051(){
    const side=$('.side-panel'); if(!side) return null;
    let tabs=side.querySelector(':scope > .mobile-menu-tabs, :scope > .side-tabs') || $('.mobile-menu-tabs,.side-tabs');
    if(tabs && tabs.parentElement!==side) side.insertBefore(tabs, side.firstChild);
    let content=$('#mbhMenuContent051') || $('#mbhMenuContent050') || $('#mbhMenuContent044') || $('#mbhMenuContent041') || side.querySelector(':scope > .menu-content-fixed');
    if(!content){ content=document.createElement('div'); content.className='menu-content-fixed'; }
    content.id='mbhMenuContent051'; content.classList.add('menu-content-fixed');
    if(content.parentElement!==side){
      const footer=side.querySelector(':scope > .menu-footer, :scope > .legal-links');
      side.insertBefore(content, footer || null);
    }
    ...
    $$('[data-menu-page]', side).forEach(b=>{ b.style.pointerEvents='auto'; b.style.visibility='visible'; b.style.opacity='1'; });
    return content;
  }
  function setPage051(page){
  ...
  function wideMode(){ return window.matchMedia('(min-width:1280px) and (min-height:701px)').matches; }
  function apply051(){
    syncVersion051();
    const side=$('.side-panel'); const btn=$('#equipToggleBtn'); if(!side) return;
    ensureMenu051();
    if(wideMode()){
      side.classList.remove('open');
      side.style.display='grid'; side.style.position='fixed'; side.style.pointerEvents='auto'; side.style.zIndex='2147483000';
      if(btn){ btn.style.display='none'; btn.textContent='メニュー'; }
      if(typeof state!=='undefined') state.uiOpen=false;
    }else{
    ...
    setPage051((typeof state!=='undefined' && state.menuPage) || 'stats');
  }
  function bind051(){
    const side=$('.side-panel'); if(!side) return;
    if(!document.__mbh051CaptureBound){
      document.__mbh051CaptureBound=true;
      const route=(e)=>{
        const t=e.target && e.target.closest ? e.target.closest('[data-menu-page],#creditBtn,#termsBtn,#privacyBtn,#equipToggleBtn') : null;
        if(!t) return;
        if(t.dataset && t.dataset.menuPage){
          e.preventDefault(); e.stopPropagation();
          setPage051(t.dataset.menuPage || 'stats');
          safe(()=>{ if(typeof playUiClick==='function') playUiClick(); });
          return false;
        }
        if(t.id==='creditBtn'){ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal('credit'); if(typeof playUiClick==='function') playUiClick(); }); return false; }
        if(t.id==='termsBtn'){ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal('terms'); if(typeof playUiClick==='function') playUiClick(); }); return false; }
        if(t.id==='privacyBtn'){ e.preventDefault(); e.stopPropagation(); safe(()=>{ if(typeof openLegalModal==='function') openLegalModal('privacy'); if(typeof playUiClick==='function') playUiClick(); }); return false; }
        if(t.id==='equipToggleBtn' && !wideMode()){
          e.preventDefault(); e.stopPropagation();
          side.classList.toggle('open');
          if(typeof state!=='undefined') state.uiOpen=side.classList.contains('open');
          apply051();
      ...
      document.addEventListener('touchstart', route, {capture:true, passive:false});
    }
    $$('[data-menu-page]').forEach(b=>{
      if(b.__mbh051Bound) return; b.__mbh051Bound=true;
      const h=(e)=>{ e.preventDefault(); e.stopPropagation(); setPage051(b.dataset.menuPage||'stats'); return false; };
      b.addEventListener('click', h, true);
      b.addEventListener('pointerup', h, true);
    });
  }
  function boot051(){ apply051(); bind051(); setTimeout(apply051,80); setTimeout(apply051,400); setTimeout(apply051,1200); }
...
/* MBH ver.0.5.3: menu closed-by-default finalizer. No MutationObserver, no BGM assets. */
(function(){
  'use strict';
  const BUILD='0.5.3';
  const $=(s,r=document)=>r.querySelector(s);
...
/* MBH ver.0.5.3: final stable menu controller. No MutationObserver, no BGM assets. */
(function(){
  'use strict';
  const BUILD='0.5.3';
  ...
  function ensureMenuContent(){
    const side=$('.side-panel');
    if(!side) return null;
    const tabs=side.querySelector('.mobile-menu-tabs,.side-tabs');
    const footer=side.querySelector(':scope > .legal-links, :scope > .menu-footer') || $('.legal-links.menu-footer') || $('.menu-footer');
    let content=byId('mbhMenuContent053') || side.querySelector(':scope > .menu-content-fixed');
    if(!content){ content=document.createElement('div'); }
    content.id='mbhMenuContent053';
    content.className='menu-content-fixed';
    if(tabs && content.previousElementSibling !== tabs){ tabs.insertAdjacentElement('afterend', content); }
    else if(!content.parentElement){ side.appendChild(content); }
    ...
    if(footer){ footer.classList.add('menu-footer'); footer.classList.add('legal-links'); }
    side.querySelectorAll(':scope > .menu-col').forEach(el=>{ el.style.display='none'; });
    return content;
  }

  function activePage(){
    const known=['stats','equip','inventory'];
    let p='stats';
    if(typeof window.state !== 'undefined' && state && known.includes(state.menuPage)) p=state.menuPage;
    return p;
  }

  function setPage(page){
    page = ['stats','equip','inventory'].includes(page) ? page : 'stats';
    safe(()=>{ if(typeof state !== 'undefined') state.menuPage = page; });
    const content=ensureMenuContent();
    $$('.side-panel [data-menu-page]').forEach(btn=>btn.classList.toggle('active', btn.dataset.menuPage===page));
    if(content){
      const map={stats:'.hero-stats', equip:'.equip-panel', inventory:'.inventory-panel'};
      Array.from(content.children).forEach(p=>p.classList && p.classList.remove('active-page'));
      const target=content.querySelector(map[page]);
      if(target) target.classList.add('active-page');
    }
  }
  ...
  function setMenuOpen(open){
    ...
    if(!side) return;
    const next=!!open;
    safe(()=>{ if(typeof state !== 'undefined') state.uiOpen = next; });
    side.classList.toggle('open', next);
    document.body.classList.toggle('mbh-menu-open', next);
    document.body.classList.toggle('menu-open', next);
    const btn=byId('equipToggleBtn');
    if(btn) btn.textContent = next ? '閉じる' : 'メニュー';
    if(next) setPage(activePage());
  }

  function bindMenu(){
    const btn=byId('equipToggleBtn');
    if(btn && btn.dataset.mbh053Bound !== '1'){
      btn.dataset.mbh053Bound='1';
      btn.onclick=null;
      btn.addEventListener('click', (e)=>{
        e.preventDefault(); e.stopPropagation();
        setMenuOpen(!document.body.classList.contains('mbh-menu-open'));
        safe(()=>{ if(typeof startAudio==='function') startAudio(); if(typeof playUiClick==='function') playUiClick(); });
      }, true);
    }
    $$('.side-panel [data-menu-page]').forEach(b=>{
      if(b.dataset.mbh053Bound==='1') return;
      b.dataset.mbh053Bound='1';
      b.onclick=null;
      b.addEventListener('click',(e)=>{
        e.preventDefault(); e.stopPropagation();
        setPage(b.dataset.menuPage || 'stats');
        safe(()=>{ if(typeof playUiClick==='function') playUiClick(); });
      }, true);
    });
  }

  function installLegalFallback(){
    const modal=byId('legalModal'), title=byId('legalModalTitle'), body=byId('legalModalBody');
    const contents={
      credit:['クレジット','<p><b>Mini Browser Hero</b></p><p>© 2026 もみヒゲ</p><p>本作品の一部素材・プログラム・文章・画像の制作には、生成AIを利用しています。</p>'],
      terms:['利用規約','<p>本作品は無保証で提供されます。</p><p>本作品の利用によって発生した損害について、制作者は一切の責任を負いません。</p>'],
      privacy:['プライバシーポリシー','<p>セーブデータはブラウザのローカルストレージに保存されます。</p><p>アクセス解析を利用する場合があります。</p>']
    };
    function open(type){
      if(!modal || !title || !body) return;
      const c=contents[type]||contents.credit;
      title.textContent=c[0]; body.innerHTML=c[1]; modal.classList.remove('hidden'); document.body.classList.add('modal-open');
    }
    [['creditBtn','credit'],['termsBtn','terms'],['privacyBtn','privacy']].forEach(([id,type])=>{
      const el=byId(id); if(!el || el.dataset.mbh053Bound==='1') return;
      el.dataset.mbh053Bound='1';
      el.onclick=null;
      el.addEventListener('click',(e)=>{ e.preventDefault(); e.stopPropagation(); open(type); safe(()=>{ if(typeof playUiClick==='function') playUiClick(); }); }, true);
    });
    const close=byId('legalModalClose');
    if(close && close.dataset.mbh053Bound!=='1'){
      close.dataset.mbh053Bound='1';
      close.addEventListener('click',(e)=>{ e.preventDefault(); e.stopPropagation(); modal?.classList.add('hidden'); document.body.classList.remove('modal-open'); }, true);
    }
    modal?.addEventListener('click',(e)=>{ if(e.target && e.target.dataset && e.target.dataset.legalClose){ modal.classList.add('hidden'); document.body.classList.remove('modal-open'); } }, true);
  }

  function repairBattle(){
    const bg=$('.battle-bg');
    if(bg){
      const current = bg.style.backgroundImage || getComputedStyle(bg).backgroundImage || '';
      if(!current || current === 'none') bg.style.backgroundImage = "url('assets/bg_forest_ruins.jpg')";
      bg.style.display='block'; bg.style.opacity='1'; bg.style.visibility='visible';
    }
    const battle=$('.battle-panel');
    if(battle){ battle.style.display='block'; battle.style.opacity='1'; battle.style.visibility='visible'; }
  }
  ...
  function boot(){
    ...
    ensureMenuContent();
    bindMenu();
    installLegalFallback();
    setPage(activePage());
    // 起動直後は必ず閉じる。既存処理がopenを付けていてもここで戻す。
    setMenuOpen(false);
    repairBattle();
    // 古い遅延パッチの後にもう一度だけ整える。監視ループは使わない。
    setTimeout(()=>{ syncVersion(); ensureMenuContent(); bindMenu(); setPage(activePage()); setMenuOpen(false); repairBattle(); }, 250);
    setTimeout(()=>{ syncVersion(); ensureMenuContent(); bindMenu(); setPage(activePage()); repairBattle(); }, 1200);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot, {once:true}); else boot();
  window.addEventListener('load', ()=>setTimeout(()=>{ syncVersion(); ensureMenuContent(); bindMenu(); setPage(activePage()); repairBattle(); }, 0), {once:true});
})();
